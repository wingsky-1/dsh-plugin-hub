...
→ S4 落地计划 → S5 对抗性评审 → （可选）S6 报告沉淀 → S7 评审→实施交接（§7，评审后进入实施时）→ 转实施 skill
```

阶段号 S0-S7（正文节号见下 S0-S7 与交付节）；S7 只在「评审结论获批进入实施」时走，平时评审止于 S6。
...
## 4. S4 落地计划（把发现改为可执行排期）

1. 阶段式排期（注：此处 P0-P4 为实施阶段编号，不同于严重度 P0-P3）：**P0 认知对齐/文档 → P1 正确性修复 → P2 场景适配 →
...
## 7. S7 评审→实施交接（指针，不内联实施细节；仅获批进入实施时走）
