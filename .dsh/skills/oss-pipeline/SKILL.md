...
> CI 落点三维度与旧称映射见 [references/gate-reading.md](references/gate-reading.md)；完成定义不读作「CI 已验全仓 test」（见根 AGENTS.md 门禁矩阵）。
...
② **隔离**：`git worktree add -b task/<n> /mnt/ssd/worktree/dsh-plugin-hub-task-<n>`（建前
   `git worktree list` 确认）；禁止多任务共用同一 checkout；
...
把失败上下文落盘 `.dsh/retro/history.jsonl`（字段 schema 与冷却口径见 oss-report 回顾段），回顾剖析与规程修订提案走 oss-report
