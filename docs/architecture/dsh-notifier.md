...
- SSE 连接表**不设连接数上限**：表的大小只由两路超窗回收收敛——stalled（写被拒连续超 90s）与
  maxAge 轮换（存活超 120min 且无业务帧）；这里的连接数含义是**服务端未释放句柄数**而非在线设备数，
  半开连接（设备息屏 / 切网 / NAT 静默掐断）不发 FIN、close/error 不触发，只能等这两路回收；
