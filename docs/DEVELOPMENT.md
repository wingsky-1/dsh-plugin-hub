...
- `pnpm contract`（contract-check）：load id === 包名、`dsh.client ⇒ exports["./client"]`、
    ...
    **叶子粒度**（递归含 `interface.ts` 的目录，分组层透明）划分，跨模块引用只能走目标
    ...
    - **数据层通道（不经台账、无 reason/reviewBy、不进到期台账；「登记即声明」，
      ...
      · `testLayers.coverageExcludes`：把文件写进该包的覆盖排除面 ⇒ 该文件退出
        ...
        现有 5 条**全部是证据级条目**（#769 的 2 条 §5.3 client 存量 + #767 的 3 条 I8①
        跨包存量），本通道（覆盖率排除面）自身零条目。条目形状与 vitest 面
  ...
  - **测试导入面判据（I8①，#767 B0 切片 3b；与上一条同属本门禁，不新增 workflow）**：
    ...
    （随搬迁清零），其他包 3 条（lan-proxy 2 / web-file-preview 1）走 `gate-exemptions.json`
    的证据级条目（trackingIssue #767）。
