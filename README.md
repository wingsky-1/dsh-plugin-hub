...
| 包名 | 功能 | 状态 |
|---|---|---|
| `@wingsky-1/dsh-gzip` | /api 响应 gzip 压缩 | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-idle-archive` | 会话闲置提醒归档 | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-notifier` | 审批/完成/错误事件通知（浏览器 Notification + 系统 toast） | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-skill-explorer` | 技能中心：分级展示 + 创建/启用/删除 skill | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-opencode-usage` | OpenCode Go 套餐用量悬浮框（rolling/weekly/monthly） | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-mcp-manager` | MCP 服务器管理器（stdio/HTTP，工具注册给模型） | ✅ 已迁入（待发布） |
| `@wingsky-1/dsh-lan-proxy` | 局域网访问 dsh web UI（HTTP/HTTPS/WS 转发 + TLS） | ✅ 已迁入（待发布） |
