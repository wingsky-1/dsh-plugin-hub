...
| `legacy-settings-migration.js` | 宿主 | 旧 `settings.yaml` / `settings.yaml.imported` section 的读取、字段级合并、JSON 安全克隆；canonical 写入前创建 pending receipt，成功后 promote 为完成 marker；未知失败恢复时不重放旧值，明确 revision 冲突才可清理重试；插件通过参数提供 namespace、sanitizer、scope、revision 与路径 |
