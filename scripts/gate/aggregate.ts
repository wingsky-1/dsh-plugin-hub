#!/usr/bin/env node
"use strict";

/**
 * aggregate — 聚合包（dsh-plugins-all）cordis.patch.yml 生成/校验器。
 *
 * 语义（对齐 web-ui aggregate 实测做法 + 本仓 ui-<name> patch id 决策）：
 *   - 聚合 cordis.patch.yml = 各独立包 insert 行**原样拼接**（id + name，**不带
 *     config**——schema 默认值兜底：lan-proxy host/port、mcp-manager
 *     announceToAgent、各插件 enabled 均有默认）；
 *   - 聚合行 id 与独立行 id 相同（ui-<目录名>）：同一插件以独立+聚合两种方式
 *     同时安装时，loader 层按 id 出现 duplicate -> TypeError（fail-loud 启动
 *     失败，明确可发现）；因此 **README 声明禁双装**（任选一种安装方式）。
 *     保持 id 相同而非另加前缀——避免「不同 id 双 entry 各自 apply 的静默双激活」。
 *   - AUTO-GENERATED 头，禁止手改；`--check` 模式漂移即失败（CI 门禁）。
 *
 * 用法：
 *   node scripts/gate/aggregate.ts           # 重新生成聚合 patch
 *   node scripts/gate/aggregate.ts --check   # 校验漂移（CI 用）
 */
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import {
  checkAggregateConsistency,
  listPluginDirs,
  loadManifest,
  warnUnknownEntries,
} from "../lib/plugins-manifest-lib.ts";

const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const CHECK = process.argv.includes("--check");
const AGGREGATE_NAME = "dsh-plugins-all";
const PATCH_PATH = join(ROOT, "packages", AGGREGATE_NAME, "cordis.patch.yml");

/** 解析子包 cordis.patch.yml 的 insert 行 -> [{ id, name }]（丢弃 config/注释）。 */
function parsePatchRows(child: string): { id: string; name: string }[] {
  const path = join(ROOT, "packages", child, "cordis.patch.yml");
  if (!existsSync(path)) {
    console.error(`[aggregate] ${child}: 缺 cordis.patch.yml（新插件请先创建 patch 再跑本脚本）`);
    process.exit(1);
  }
  const text = readFileSync(path, "utf8");
  const rows: { id: string; name: string }[] = [];
  for (const line of text.split("\n")) {
    const m = line.match(/^\s*-\s+id:\s*(\S+)/);
    if (m) {
      const nameM = /^\s*name:\s*['"]?([^'"]+)['"]?\s*$/.exec(
        text.split("\n")[text.split("\n").indexOf(line) + 1] ?? "",
      );
      rows.push({ id: m[1], name: nameM ? nameM[1] : "" });
    }
  }
  return rows;
}

/** 生成聚合 cordis.patch.yml 内容（子包行原样拼接，不带 config）。 */
function renderPatch(children: string[]): string {
  const lines = [
    "# AUTO-GENERATED by scripts/gate/aggregate.ts — do not edit",
    "# 聚合行 = 各独立包 cordis.patch.yml 的 insert 行原样拼接（id + name，无 config，",
    "# 配置走 schema 默认值）。改独立包 patch 后运行 `node scripts/gate/aggregate.ts` 重新生成。",
    "- insert:",
  ];
  for (const child of children) {
    const rows = parsePatchRows(child);
    if (rows.length === 0) {
      console.error(`[aggregate] ${child}: 未解析到 insert 行`);
      process.exit(1);
    }
    for (const { id, name } of rows) {
      lines.push(`    - id: ${id}`);
      lines.push(`      name: '${name}'`);
    }
  }
  return lines.join("\n") + "\n";
}

const children = listPluginDirs(ROOT);

// 清单一致性前置闸：目录集必须 == manifest.active ∪ standalone 集（issue #36，opt-in fail-closed）
warnUnknownEntries(ROOT);
let manifest: ReturnType<typeof loadManifest>;
try {
  manifest = loadManifest(ROOT);
} catch (e) {
  console.error(`[aggregate] ${(e as Error).message}`);
  process.exit(1);
}
const manifestProblems = checkAggregateConsistency({ dirNames: children, manifest });
if (manifestProblems.length > 0) {
  for (const p of manifestProblems) console.error(`[aggregate] FAILED: ${p}`);
  process.exit(1);
}

// 聚合 patch 只收 active 集；standalone（独立发包，如 demo 演进包）不进聚合。
const aggregateChildren = children.filter((d: string) => manifest.active.includes(d));
const generated = renderPatch(aggregateChildren);

if (CHECK) {
  const onDisk = existsSync(PATCH_PATH) ? readFileSync(PATCH_PATH, "utf8") : "";
  if (onDisk !== generated) {
    console.error(
      `[aggregate] check FAILED: ${AGGREGATE_NAME}/cordis.patch.yml 与生成内容漂移——运行 node scripts/gate/aggregate.ts`,
    );
    process.exit(1);
  }
  console.log(`[aggregate] check OK: ${AGGREGATE_NAME}（${children.length} 个子包，行数一致）`);
  process.exit(0);
}

writeFileSync(PATCH_PATH, generated);
console.log(
  `[aggregate] 已生成 ${AGGREGATE_NAME}/cordis.patch.yml（${children.length} 个子包行原样拼接，无 config）`,
);
