...
export function checkLedger(ledger, expected) {
  ...
  for (const m of ledger.measurements ?? []) {
    ...
    if (typeof m.scope !== "string" || m.scope.trim() === "")
      problems.push(`run ${m.run?.id}: scope 缺失`);
...
function main() {
  ...
  if (isCheck) {
    ...
    if (problems.length > 0) {
      console.error(
        `[ledger] --check 失败：${problems.length} 项（拆段/加包后必须重测并更新台账）`,
      );
      return 1;
    }
    ...
    console.log(
      `[ledger] --check 通过：覆盖全部 ${cur.size} 段` +
        ...
        (historical.length > 0
          ? `；历史段 ${historical.length} 条已在 superseded 登记取代关系`
          : ""),
    );
    return 0;
  }
  ...
  if (parsed.length === 0) {
    console.error(
      `[ledger] 日志中未解析到任何 stryker 段：${fromLog}（run 未跑变异，或日志格式已变）`,
    );
    return 2;
  }
  ...
  if (argv.includes("--write")) {
    ...
    console.log(
      `[ledger] 已写入 ${LEDGER_PATH}：run ${entry.run.id} ${segments.length} 段（scope=${scope}）`,
    );
  } else {
