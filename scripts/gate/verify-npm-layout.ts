...
if (retiredDirs.length > 0) {
  console.warn(
    `[verify-npm-layout] 跳过已退役包残留目录: ${retiredDirs.join(", ")}（manifest.retired 已登记，请清理）`,
  );
}
...
for (const p of allPackages) {
  ...
  if (bad.length > 0) {
    failed++;
    console.log(
      `FAIL ${pkgJson.name} | 发布包不得携带安装期脚本: ${bad.join(", ")}（构建改用 prepublishOnly）`,
    );
  }
}
...
if (scoped !== null) {
  console.log(
    `[verify-npm-layout] 解包切片：${layoutTargets.length}/${plugins.length} 包（--packages ${scoped.join(",") || "（空）"}）`,
  );
}
  ...
  try {
    ...
    if (!existsSync(idxPath)) {
      ...
      const declaresClient =
        JSON.parse(readFileSync(join(pkgRoot, "package.json"), "utf8")).dsh?.client !== undefined;
      if (
        (existsSync(join(pkgRoot, "lib", "client.js")) || declaresClient) &&
        !/ROUTES|route/i.test(modText)
      ) {
    ...
    if (
      existsSync(idxPath) &&
      /(?:from|import)\s*["']\.\.\/\.\.\/(?:shared|types)/.test(readFileSync(idxPath, "utf8"))
    ) {
    ...
    console.log(
      `${problems.length === 0 ? "PASS" : "FAIL"} ${name} | ${problems.join("; ") || "npm 布局可加载"}`,
    );
  } catch (e) {
    failed++;
