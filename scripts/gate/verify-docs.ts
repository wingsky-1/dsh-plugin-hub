...
function checkAgentDocs(): number {
  const docs = walkAgentDocs(AGENT_ROOT, [])
...
function checkPkg(pkgDir: string): string | undefined {
  ...
  for (const target of relLinks(md)) {
    if (checkRelTarget(readme, target) !== null)
      failures.push(`${name}: README 相对链接目标缺失 ${target}`);
  }
  // README.en.md 配对
  ...
  if (existsSync(en)) {
    ...
    for (const target of relLinks(emd)) {
      if (checkRelTarget(en, target) !== null)
        failures.push(`${name}: README.en 相对链接目标缺失 ${target}`);
    }
  } else if (STRICT_EN) {
  ...
  if (/__NAME__|__PLUGIN__|<name>|TODO/.test(desc))
    failures.push(`${name}: description 含脚手架占位符`);
...
if (existsSync(rootEn)) {
  ...
  for (const target of relLinks(emd)) {
    if (checkRelTarget(rootEn, target) !== null)
      failures.push(`根 README.en 相对链接目标缺失 ${target}`);
  }
} else if (STRICT_EN) {
