...
- **manifest 必须与树逐文件对齐**：两个写入方都受 `pushBaselineTree` 的**推送前双向校验**保护
  （缺条目、多条目都直接抛错、不推送）。这一条是 2026-09-12 生产实测补上的：并集入档首夜
  manifest 只写了「新算」的 31 段、漏掉「沿用」的 2 段——树是对的（33 段没缩水）但 manifest 少 2 条，
  而 manifest 是留段时间戳与后续完整性校验的唯一依据。
- **远端 manifest 缺条目时自愈**：按远端 blob 就地重算 `size` / `sha256`，`mtime` **记 `null`**
  （未知即未知，不冒充「刚刚测过」——陈旧判据必须能区分「未知」与「新鲜」），并打 `::warning::` 点名。
  不选 fail-loud：文件内容就在树里，条目缺失是可恢复的；拒绝入档只会让归档停在旧状态，
  而「谁也推不上去」正是本方案要消灭的失效形态。
