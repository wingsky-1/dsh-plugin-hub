...
test("#3a 归一化极端：同文件内引用深度 ≠ 文件深度 → 全部按文件目录深度归一为固定 prefix", () => {
  ...
  assert.match(
    out,
    /import\('\.\.\/shared\/dyn\.js'\)/,
    "d=0 动态 import 深度 2 → ../shared/dyn.js",
  );
...
function makeLibTree(root) {
  ...
  writeFileSync(
    join(root, "a", "b", "c", "deep.d.ts"),
    "export { d } from '../../../../shared/w.js';\n",
  );
  // 混入非 .d.ts 与无关引用（不得触碰）
...
test("#4 目录级集成：mkdtemp 目录树递归遍历按深度改写（d=0/1/2/3 全覆盖）", () => {
  ...
  try {
    ...
    assert.match(
      out[join(lib, "index.d.ts")],
      /from '\.\.\/shared\/x\.js'/,
      "d=0 → ../shared/（1 个 ../）",
    );
    assert.match(
      out[join(lib, "client", "x.d.ts")],
      /from '\.\.\/\.\.\/shared\/y\.js'/,
      "d=1 → ../../shared/（2 个 ../）",
    );
    assert.match(
      out[join(lib, "a", "b", "c.d.ts")],
      /from '\.\.\/\.\.\/\.\.\/shared\/z\.js'/,
      "d=2 → ../../../shared/（3 个 ../）",
    );
    assert.match(
      out[join(lib, "a", "b", "c", "deep.d.ts")],
      /from '\.\.\/\.\.\/\.\.\/\.\.\/shared\/w\.js'/,
      "d=3 → 4 个 ../",
    );
    // 非 .d.ts / 非目标文件不得改写
    assert.match(
      out[join(lib, "notes.txt")],
      /from '\.\.\/\.\.\/shared\/ignored\.js'/,
      "非 .d.ts 文件不改写",
    );
...
test("#6 2b 复制含深层子目录：shared/<d>/<d>/x.d.ts 连同目录结构原样复制进包", () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(join(pkgDir, "shared", "deep", "deeper", "x.d.ts")),
      "2 级子目录 .d.ts 连同目录结构复制",
    );
    assert.ok(!existsSync(join(pkgDir, "shared", "deep", "sample.js")), "非 .d.ts 不复制");
    // 与 listSharedDts 同源枚举一致（复制清单 == 期望清单）
    assert.deepEqual(
      walkFiles(join(dir, "shared"), (f) => f.endsWith(".d.ts")),
...
test("#4b 真实仓库方向：bundle-host 2b 复制谓词与 listSharedDts 同源且含深层子目录", () => {
  ...
  try {
    ...
    assert.ok(
      expected.some((rel) => rel.includes("/")),
      "清单含子目录路径（深层复制面）",
    );
