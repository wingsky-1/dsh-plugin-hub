 ...
 *     `tools`（lint 工具链入口）。口径 =「门禁系统以路径字符串直接执行的脚本所在目录」；`scripts/lib`
 *     （只被 import 的共享库，删改会让 import 方在 tsc / node 解析上响亮失败）、`scripts/build`
 *     （由各包 package.json 的 build 别名调用，门禁档位不展开 `pnpm -r build`，实测 0/4 有执行点）、
 ...
 *     又会打开「判据的偶然 import 即免死」的侧门（见 REACHABLE_LIBRARIES 注释）。
 *   - 不进面：`scripts/build` / `scripts/maintenance` / `scripts/test`。构建链由各包
 *     package.json 的 build 别名调用（门禁档位不展开 `pnpm -r build`，实测 0/4 有执行点），
 *     维护脚本按需手工执行，自测面的执行者是 glob。这三处是**已登记的口径边界**；
 *     「各包 build 必须含 clean-lib + bundle-host」要的是另一条形断言（#843 M9 的另一半），
 *     不在本面的论域内。
 *
