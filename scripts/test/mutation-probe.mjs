...
function usage(msg) {
  ...
  console.error(
    '用法: node scripts/test/mutation-probe.mjs --report <报告路径> [--max 8] [--run "<命令>"] [--filter <正则>] [--seed <n>] [--list] [--fail-on-miss]',
  );
  process.exit(1);
}
...
const survived = all
  ...
  .sort((a, b) =>
    a._file === b._file ? Number(a.id) - Number(b.id) : a._file.localeCompare(b._file),
  );

console.log(`mutation-probe: ${reportPath}`);
console.log(
  `  变异体 ${all.length} 个；Survived ${all.filter((m) => m.status === "Survived").length} 个；本次抽样 ${Math.min(max, survived.length)} 个`,
);
...
const dirty = execSync("git status --porcelain -- " + targets.map((f) => `"${f}"`).join(" "), {
  cwd: ROOT,
  encoding: "utf8",
}).trim();
if (dirty !== "") {
