 ...
 * 取文件 mtime 当「新鲜度」会把基线写入方的文件重写误判成重测（已退役的增量班次即每次重写
 * 全部段文件）。故这里锁死：
...
const FIXTURE = [
  ...
  line(
    "2026-09-11T08:00:01.0000000",
    "\u001b[32mINFO Instrumenter\u001b[39m Instrumented 1 source file(s) with 100 mutant",
  ),
  line(
    "2026-09-11T08:00:02.0000000",
    "\u001b[32mINFO DryRunExecutor\u001b[39m Initial test run succeeded. Ran 7 tests in 3 seconds",
  ),
  line(
    "2026-09-11T08:00:03.0000000",
    "\u001b[32mINFO IncrementalDiffer\u001b[39m Incremental report:",
  ),
  line("2026-09-11T08:00:04.0000000", "40 of 100 mutant result(s) are reused."),
  line(
    "2026-09-11T08:00:05.0000000",
    "\u001b[32mINFO MutationTestExecutor\u001b[39m Done in 13 seconds.",
  ),
  line("2026-09-11T08:00:06.0000000", "##[endgroup]"),
  // 未闭合的段（被杀）：不得出现在结果里
  line("2026-09-11T08:00:07.0000000", "##[group]stryker dsh-x-b"),
  line(
    "2026-09-11T08:00:08.0000000",
    "\u001b[32mINFO Instrumenter\u001b[39m Instrumented 1 source file(s) with 50 mutant",
  ),
].join("\n");
...
const MATRIX_FIXTURE = [
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:00:00.0000000",
    '##[group]Run npx stryker run "stryker.conf.d/dsh-x.json"',
  ),
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:00:01.0000000",
    "\u001b[32m08:00:01 (123) INFO ProjectReader\u001b[39m Found 1 of 10 file(s)",
  ),
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:00:02.0000000",
    "\u001b[32m08:00:02 (123) INFO Instrumenter\u001b[39m Instrumented 1 source file(s) with 50 mutant",
  ),
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:00:04.0000000",
    "\u001b[32m08:00:04 (123) INFO DryRunExecutor\u001b[39m Initial test run succeeded. Ran 3 tests in 2 seconds",
  ),
  // 进度行：progress-append-only reporter，不带 (pid) INFO —— 只认 INFO 会在此截断
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:00:30.0000000",
    "Mutation testing 50% (elapsed: ~30s, remaining: ~30s) 15/30 tested",
  ),
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:01:00.0000000",
    "Mutation testing 90% (elapsed: ~59s, remaining: ~7s) 27/30 tested",
  ),
  shardLine(
    "dsh-x",
    "UNKNOWN STEP",
    "2026-09-12T08:01:05.0000000",
    "##[group]Upload shard reports",
  ),
].join("\n");
...
test("矩阵形态：无段内输出的实例不产出记录（不得用 0 或 job 墙钟冒充）", () => {
  const noOutput = shardLine(
    "dsh-x",
    "Install dependencies",
    "2026-09-12T08:00:00.0000000",
    "Progress: resolved 505",
  );
...
test("wallSeconds 必须是正数（解析失败不得当作有效测量写入台账）", () => {
  assert.deepEqual(
    checkLedgerEntry({ seg: "x", wallSeconds: 1.5, mutants: 10, reused: null, reuseTotal: null }),
    [],
  );
  assert.ok(
    checkLedgerEntry({ seg: "x", wallSeconds: 0, mutants: 10 }).some((p) =>
      p.includes("wallSeconds"),
    ),
  );
  assert.ok(
    checkLedgerEntry({ seg: "x", wallSeconds: -1, mutants: 10 }).some((p) =>
      p.includes("wallSeconds"),
    ),
  );
  assert.ok(
    checkLedgerEntry({ seg: "x", wallSeconds: 1, mutants: 10, reused: 5, reuseTotal: 3 }).some(
      (p) => p.includes("reused"),
    ),
  );
});
