...
import {
  checkCordisMergeReachability,
  closureDeclaresCordisMerge,
  collectDtsClosure,
  detectCordisServiceProvide,
  srcDeclaresCordisMerge,
} from "../lib/dts-cordis-merge-lib.ts";
...
test("真实包：dsh-notifier 产物闭包命中合并（合并已迁入 src/index.ts，#733 M2）", () => {
  ...
  assert.deepEqual(
    srcHits.map((f) => f.slice(pkg.length + 1)),
    ["src/index.ts"],
  );
