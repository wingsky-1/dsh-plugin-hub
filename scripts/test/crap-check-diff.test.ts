...
test("findBaseFunction: 支持行号映射与同名函数查找", () => {
  ...
  // 3. 全新函数返回 null
  const matchC = findBaseFunction(
    { name: "brandNewFunc", startLine: 50, endLine: 60 },
    baseFns,
    hunks,
  );
...
`;
    ...
    const res = spawnSync(process.execPath, [SCRIPT, "--diff", "HEAD"], {
      cwd: dir,
      encoding: "utf8",
    });
...
`;
    ...
    assert.equal(
      res.status,
      0,
      `未触及的存量超标函数必须豁免 exit 0，实际输出：${res.stdout} ${res.stderr}`,
    );
    assert.match(res.stdout, /全部合规放行/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
