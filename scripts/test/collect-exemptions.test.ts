...
test("本仓真实快照：9 条在册（数字变即提示同步台账与 #765）", () => {
  ...
  // #767 B0 切片 3a：§5.3 判据上线时 notifier 的 2 条 client → src/server 存量同样必须进台账
  // （逐条钉住**证据键**本身，而不是只钉一个总数：条目被换成人手写的近似路径时先红）。
  assert.match(r.stdout, /trackingIssue #769/);
  assert.match(
    r.stdout,
    /\$\.exemptions\[2\] {2}gate=verify-dir-imports {2}path=dsh-notifier:src\/client\/capabilities\.ts\|src\/server\/channels\/impl\/capabilities\/type\.ts/,
  );
  assert.match(
    r.stdout,
