...
test("本仓真实快照：5 条在册（数字变即提示同步台账与 #765）", () => {
  // 5 = coverage.config.json 4（#769 把一条 **/client/** 拆成 per-package 的 pending-project
  ...
  //     gate-exemptions.json 的 #843 M-1（dsh-worktree-sidebar 缺回落锚点）已随本 PR 回填基线删除，
  //     故 exemptions 为空，不再计入。
  const r = spawnSync(process.execPath, [SCRIPT], { cwd: ROOT, encoding: "utf8" });
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.stdout, /合计 5 条：已过期 0 /);
