...
test("#718: overlay-baseline 恢复段——探针说 present 但拉取失败时 fail-loud（exit 1）", () => {
  ...
  try {
    ...
    writeFakeGh(tmp, {
      ...
      runsOk: true,
      artifactsOk: true,
      artifacts: ONE_MUTATION_ARTIFACT,
    });
...
test("#572: orphan-baseline 本地 push 与 restore 往返完整性", () => {
  ...
  try {
    ...
    writeFileSync(
      join(targetDir, "incremental-test-pkg.json"),
      '{"mutants":[{"id":1,"status":"Killed"}]}',
    );
    writeFileSync(
      join(targetDir, "incremental-test-pkg-2.json"),
      '{"mutants":[{"id":2,"status":"Survived"}]}',
    );
    ...
    const branchCheck = execFileSync(
      "git",
      ["rev-parse", "--verify", "refs/heads/baseline/mutation"],
      {
        cwd: tmp,
        encoding: "utf8",
        stdio: "pipe",
...
function readArchived(repo: string, name: string): string {
  return execFileSync("git", ["show", `refs/heads/baseline/mutation:${name}`], {
    cwd: repo,
    encoding: "utf8",
  });
}
...
test("#718 S1.2: archive 并集入档——本次未产出的段沿用远端，段数不缩水", () => {
  ...
  try {
    ...
    assert.deepEqual(
      archivedFiles(tmp),
      [
        "incremental-alpha.json",
        "incremental-beta.json",
        "incremental-gamma.json",
        "manifest.json",
      ],
      "并集入档后段的集合不得缩水（旧实现会在这里抹掉 beta/gamma）",
    );
    assert.equal(
      readArchived(tmp, "incremental-beta.json"),
      '{"seg":"beta"}',
      "沿用段保留远端内容",
    );
    ...
    const tag = execFileSync("git", ["tag", "-l", "baseline-snap-*"], {
      cwd: tmp,
      encoding: "utf8",
    }).trim();
...
test("#718 S2.1: overlay 端到端——差量覆盖生效 + 沿用段保留旧 manifest 条目 + 回滚快照", () => {
  ...
  try {
    ...
    assert.equal(
      readArchived(tmp, "incremental-alpha.json"),
      '{"seg":"alpha","gen":2}',
      "覆盖段换成新内容",
    );
    assert.equal(
...
function rewriteRemoteManifest(repo: string, keepNames: string[]): void {
  ...
  const commit = execFileSync("git", ["commit-tree", tree, "-m", "test: trim manifest"], {
    cwd: repo,
    encoding: "utf8",
  }).trim();
...
test("#718 S1.2 修复: 远端 manifest 缺沿用段条目时按 blob 重算并告警（自愈，不卡死归档）", () => {
  ...
  try {
    ...
    assert.equal(
      manifest["incremental-beta.json"].mtime,
      null,
      "重算条目 mtime 必须诚实记 null（不冒充刚测过）",
    );
    assert.equal(
      manifest["incremental-beta.json"].size,
      '{"seg":"beta"}'.length,
      "size 取自远端 blob",
    );
    assert.equal(
      typeof manifest["incremental-beta.json"].sha256,
      "string",
      "sha256 按远端 blob 原始字节重算",
    );
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
