...
test("本仓真实快照：client/index.tsx 的 7 处模块级 var 全部走登记豁免 → exit 0", () => {
  ...
  assert.equal(r.status, 0, r.stderr);
  // 包面含 dsh-lan-proxy（#765 扩面，实测零命中后才扩）与 dsh-mcp-manager（#767 B0 扩面）：
  // 范围或豁免数变了此断言先红，提示复核扩面依据与台账。
  assert.match(
    r.stdout,
    /OK（扫描 \d+ 文件，包 dsh-lan-proxy, dsh-mcp-manager, dsh-notifier，登记豁免 8 处）/,
  );
  // #767 B0 起扫描面是三包，mcp 另有 1 处文件级豁免；按行筛选后把 notifier 的 7 处钉死：
  // 数量变了、或台账条目被删，此处先红并提示同步台账与 #762。
  ...
  assert.equal(
    notifierHits.length,
    7,
    `notifier 客户端模块级 var 应有 7 处走登记豁免，实际 ${notifierHits.length}：\n${r.stdout}`,
  );
