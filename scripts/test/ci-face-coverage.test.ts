 ...
 *      文件派生出的面集合。比旧的「每个声明面都被 filters 接上」更严——少声明（漏面）与多
 *      声明（凭空面）都判红。
...
test("#742 2.3: ci-face-registry 结构合法（faces 名必须是 ci.yml 的真实面）", () => {
  ...
  for (const e of REGISTRY.entries) {
    ...
    if (e.faces !== undefined) {
      ...
      // 声明了面才需要写归属理由；无面条目的理由是推导出来的，why 由 N1/N2/N3 给出。
      assert.equal(
        e.why,
        undefined,
...
test("#875 H9: 面集合等值——登记面并集必须恰好等于 ci.yml 为该文件派生出的面（漏面与多面都判红）", () => {
  // 比旧的「每个声明面都被 filters 接上」更严：旧判据只查「声明的面接上了没有」，一个面都没
  // 声明的条目在它眼里完全合法——`scripts/test/**` 声明 faces: [] 却命中 global 面的
  // run-vitest.mjs，就是这样长期绿的。改成逐文件的集合等值后，漏面与多面都判红。
  const gaps: string[] = [];
