...
`;
  ...
  assert.deepEqual([...branch.members.base].sort(), [
    "channels",
    "errorMergeWindowMs",
    "quietHours",
  ]);
...
// 客户端 UI 引用键（EVENT_KEYS 首列 + switchControl 参数 + patch 对象键 + settings.<键>）
...
function apply(ctx: any) {
  ...
  function builtinCard(index: number, ch: any, label: string) { return null; }
  function switchControl(key: string) { return null; }
  builtinCard(0, { type: "browser" }, "x"); // 内置卡按条目渲染：实参里没有配置键
...
`;
  ...
  assert.ok(!ui.includes("browser") && !ui.includes("x"), "内置卡条目形态不贡献键面");
});
