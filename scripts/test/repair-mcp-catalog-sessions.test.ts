...
function writeV3Log(dir) {
  const header = { type: 'session', version: 3, id: 'session-test', createdAt: 1789101518091, delegationDepth: 0, cwd: '/tmp' }
  const message = { id: 'msg-1', role: 'user', content: [{ type: 'text', text: CATALOG_TEXT }], source: LEGACY_SOURCE }
  const rows = [
    header,
    { type: 'agent/inbox/spliced', seq: 0, time: 3, data: { target: 'next-turn', start: 0, inserted: [message] } },
  ]
