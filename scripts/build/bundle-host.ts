...
if (!process.argv[2]) {
  console.error(
    "用法：node scripts/bundle-host.mjs <插件目录>（如 packages/dsh-notifier，或包目录内传 .）",
  );
  process.exit(1);
}
...
if (adapterModulesCopied > 0) {
  console.log(
    `[bundle-host] ${process.argv[2]}: 内置适配器 mjs/d.mts 已拷贝进 lib/adapters/（${adapterModulesCopied} 个）`,
  );
}
...
try {
  rmSync(join(libDir, "index.js.map"));
} catch {
  /* map 不存在则跳过 */
}
...
if (clientSrc) {
  ...
  const inlineBareImports = !!(
    pkgJson.dsh &&
    pkgJson.dsh.client &&
    pkgJson.dsh.client.inlineBareImports
  );
  ...
  try {
    ...
    if (mod.ROUTES && typeof mod.ROUTES === "object") {
      ...
      console.log(
        `[bundle-host] ${process.argv[2]}: ROUTES 已注入 client 构建（${Object.keys(mod.ROUTES).length} 项）`,
      );
    }
  } catch (e) {
    console.warn(
  ...
  try {
    rmSync(join(libDir, "client.js.map"));
  } catch {
    /* 无 map 则跳过 */
  }
  ...
  if (existsSync(mermaidEntry)) {
    try {
      const { bytes, refs } = await buildMermaidChunk({
        entry: mermaidEntry,
        outfile: join(libDir, "client-mermaid.js"),
      });
    ...
    try {
      rmSync(join(libDir, "client-mermaid.js.map"));
    } catch {
      /* 无 map 则跳过 */
    }
  }
...
function cleanFreeFloatingJs(dir, isRoot) {
  for (const f of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (f.name.endsWith(".js") && !isTopEntry) {
      ...
      f.name.endsWith(".js.map") ||
      (f.name.endsWith(".map") && !(isRoot && f.name === "index.js.map"))
    ) {
      rmSync(abs, { force: true });
    }
  }
