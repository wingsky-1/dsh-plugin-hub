...
- `gate/threshold-monotonic.mjs` — 阈值单调性校验（对比 `origin/main`）：**#843 D5 起由 `data/threshold-registry.json` 的声明表驱动**——每条事实源声明 `kind`（value / boolean / baseline / existence）、方向与删键语义，比较逻辑在 `lib/threshold-registry.mjs`；**未在表里登记的事实源一律判红**（枚举口径 = 运行时读 `data/*.json`，不在表里抄第二份清单）。覆盖面：`data/coverage.config.json` 的 `thresholds`（#733 计划项 3.4 起的覆盖率面**唯一事实源**；`vitest.config.ts` 只 import 它，内联字面量会被 `verify:coverage-scope` 判红——基准侧仍可能只有 `.ts`，故该源由声明表的 sources 表达双读并走 acorn 读取器）、`gauntlet.config.json` 的变异阈值 / 逐包回落锚点 / complexity / crap / lint 预算、`mutation-topology.json` 的 `timeoutMS`、`gate-wiring-exceptions.json` 的 9 个豁免预算。 声明表**自己**也被同一套「相对基准只许补全收紧」的语义守着（评审 P0-1）：guard 只许新增，同 id 的 kind / weaken / weakenValue / onRemoval / paths / keys / anchorFields / requireFields / sources / missingIsError / nonMonotonic / minAllowed / maxAllowed 只许补全收紧（`sources` 是回落链，只许**尾部追加**：前置影子源会让两侧读到不同的事实源；比较器另记录两侧实际命中的源，工作区命中基准未声明的文件同样判红），**数据面没有任何自授权通道**（#875 H11）：`retired`（整条退役）与 `contractApprovals`（改某字段）两个键已整体删除——它们曾让「删一条 guard / 翻一个 direction / 把 onRemoval 改成 ignore」成为一行数据改动且 CI 全绿。这两个键**重现即判红**（exit 1，`lib/threshold-registry.mjs` 的 `selfAuthChannelProblems`，内容是不是空数组都一样），「不可重建」因此是被守住的而非靠人记得；改 guard 的**唯一合法路径是改判据代码**（`gate/threshold-monotonic.mjs` 或 `lib/threshold-registry.mjs`）；该路径走 PR 评审 + 本仓自测把关；`scripts/gate/threshold-monotonic.mjs` 与 `scripts/lib/threshold-registry.mjs` 都不在 `approved` 派生面内（该面恰 9 条：`.github/**`、`.dsh/skills/**`、`scripts/gate/red-line-approval.mjs`、声明表自身及其 `guards[].sources`——`scripts/gate/**` 整树并不在面内，但 `red-line-approval.mjs` 在，别按目录通配推），故不需要 `approved` 标签。 **上限（如实声明，勿误读）**：本闸保证的只是「这两个具名键不能只靠一行数据重建」；**自授权通道在类上并未消除**——实测新增一个顶层键（如 `waivers`）加约 4 行代码即可让真实判据被削弱而门禁 exit 0、`node --test` 83/83 全绿，根因是**比较器自我验证**（内置通道吸收自身全部检测，真值快照用例走同一个被削弱的比较器故一并失明），更根本地说**判据无法保护自己不被改**；收口办法是下一件 PR 的顶层键白名单。**边界（如实声明）**：面类判据不在本表——覆盖率 exclude 与变异段 mutate「相对基准的收窄」目前仍无守卫（`#848` 的判据⑤/⑥ 只保证锚定与非空，不看基准），本表只收 JSON 标量 / 布尔 / 存在性；`mutation.timeoutMS` 这类非单调旋钮的**收紧方向只报警不判红**（证据约束待设计）。
...
- `gate/verify-host-seams.mjs` — 宿主接缝结构门禁（R1-R4）：R1 事件装配点（仅 `.on`、receiver 不限名，允许 src/index 与 apply/server，根 shared 与 client 越位即红）、R2 路由三层（定义层 basename 锚定允许集、消费层调用实参须经具名表、拼接层允许集外即红）、R3 slot 装配点（仅 client/index）、R4 复刻值 (包,值) 分组（>2 红、==2 须一端为客户端）；esbuild 剥类型 + acorn + sourcemap 回映（禁纯 grep 裸判），fail-closed 经 gate-exit 唯二出口，禁 --package。执行点是 ci.yml 的 `Verify host seams` 直接步骤与本地档位的 cheapGlobal（verify-shared-fanin 之后）。
- `gate/verify-provider-usage-shape.mjs` — provider-usage 三处源码形态锁（#768 干跑红修复：源码文本断言只进 gate 层，不进 vitest）：a) `apply.ts` 内 `new ReportConfigService` 实参键集恰为 `[initial,onUpdate,root]` b) `server/history/history.ts` 不含 `dshHome`/`process.env`/`homedir` c) `server/aggregate/store.ts` 含 `deletePromises` 具名注解 `Array<Promise<void>>`；缺文件 fail-closed（exit 2），违例 exit 1，通过 exit 0。执行点是 ci.yml 的直接步骤与本地档位的 cheapGlobal。
...
- `maintenance/migrate-plugin-rows.mjs` — 一次性 profile row identity 迁移：把已发布 `ui-dsh-*` patch 行改为 `dsh-*`，默认 dry-run，`--apply` 原子写回并留备份；保留 config/disabled/name/顺序，发现重复或目标冲突时 fail-closed。settings namespace 已是 v0.2.5 的 `dsh-*`，工具不搬运 settings 文档。根脚本别名：`pnpm migrate:plugin-rows`。
...
- `maintenance/equiv-check.ts` — 等价重构的「机制性等价性检查器」（#843 M10，即 [DEVELOPMENT §4.1 验证三件套](../docs/DEVELOPMENT.md#user-content-equivalence-refactor)第 2 件）。**它是什么**：机制性抽取 + 登记表对账器，**不是判据**——把基线 ref 与改动 ref 两侧源码各解一次 AST（复用 `lib/config-matrix-lib.ts` 的 `parseTs`，不另写解析器、不引新依赖），抽取**字面量 / 非计算对象键 / 正则 / `process.exit(n)` 与 `process.exitCode = n`** 的多重集做差集，外加 §4.1 点名的两个专项（**数字字面量整段消失**防丢上界阈值、**新增函数零引用**防抽了不用），再拿登记表逐条对账：已登记 exit 0、未登记或登记项失效 exit 1、ref 不存在 / 解析失败 / 登记表本身坏了一律 exit 2（坏表不退化成「未登记」，三态比 `scan-actions-concurrency.mjs` 的 0/2 补齐了 1）。**纯文档 diff（比对面没有源码文件）走 2 属预期**，既不是判红也不是门禁故障。登记表按（文件 × 类别）分组、组内逐条枚举（值, 计数差，不接受通配符/前缀/范围）+ 每组一条理由。**它不校验理由的类别**，只校验登记项非空且与实测集合互相吻合——理由是否落在 §4.1 允许的两类内**由人负责**，逐条填 TODO 也能过；它**不对「差异是否无害」作任何判定**。**覆盖面边界（常驻 `--help` 与每次输出、`--json` 走 `boundary` 字段）：本工具不构成等价性证明**——控制流等价性不在面内（分支顺序对调、条件取反、状态少复位一次，**只要没有增删上述记号**都照样报 0 差异），**记号原样保留、只在语义上被对调**（如两个 `return` 的返回值互换）同样不可见；这些须由 §4.1 第 3 件的差分对拍证明。比对面只含源码文件，非源码面（文档 / 数据）被跳过。自测见 `test/equiv-check.test.ts`（含「把控制流回归种回去时工具仍报 0」「两个 `return` 的值互换仍报 0」两条防误用例）。维护者 / 本地工具，需 git 完整历史（要比对历史 ref），故**不进 CI**（进 CI 属 `.github/` 红线段，且维护工具不进 CI 属 AGENTS.md 明写的设计）。

## lib/（纯共享库，只被 import，不被 `node` 直接调用）

- `lib/catalog-peers-lib.ts` — catalog 与 DSH-facing peer 投影的校验 / 生成库（issue：DSH 0.1.7 兼容层直读 link 目标的原始 peerDependencies，而 pnpm 的 `catalog:` 不是 SemVer）：成员关系由 `plugins-manifest.json` 登记、版本由 `pnpm-workspace.yaml` catalog 登记，`package.json` 的 `peerDependencies` 是两者投影出的精确版本；`maintenance/sync-catalog-peers.ts` 据此写回，`gate/contract-check.ts` / `gate/pack-check.ts` / `gate/verify-npm-layout.ts` 据此断言锁版一致，成员缺失多出 / catalog range 非 exact SemVer / 坏 package.json 一律写入前 fail-closed。
...
- `lib/threshold-registry.mjs` — 阈值声明表的读取、结构与覆盖面校验、按 kind 的通用比较器（#843 D5）：两侧事实源由调用方注入（`readBase` / `readWorkspace`），`makeSourceLoader` 按 `sources` 声明顺序取第一个**存在**的源（迁移期双读的形态即由此表达，而不是特判）；`validateDeclarations` 把「新数据文件未登记 / 幽灵声明 / 缺字段」变成判据，`validateGuardFacts` 拦「声明了但两侧都取不到值」的幽灵判据；`compareDeclarationTable` 把声明表**自己**守着（guard 只许新增、判据形状只许补全收紧，无任何数据面放行通道），`selfAuthChannelProblems` 反向守住「已废止的 `retired` / `contractApprovals` 通道不可重建」（#875 H11）。
- `lib/ts-lex.ts` — TS/TSX 源码的**词法**扫描（acorn tokenizer，零新增依赖）：本仓工具链的实测硬事实是 esbuild 剥类型会擦除 `import type` 与 `declare module`（`verbatimModuleSyntax` 亦然），而本仓对官方包的引用几乎全是类型导入——故「剥类型 + acorn.parse」的 AST 里恒不存在这两类构造，只跑 AST 的判据会**恒零命中的假绿**。本模块提供词法化 + 行号 + 分段续扫（.tsx 的 JSX 区段不可词法化，抛错时跳到最近行首续扫，整体仍是一次线性扫描），被跳过的区段记进 `skipped`，调用方据此对「跳过区里恰好有 import/declare」fail-closed。机制层，不认任何门禁语义（消费者：`lib/catalog-peers-lib.ts` 的源码引用事实、`gate/forbid-session-snapshot-src.mjs` 的 B 腿）。
- `lib/gate-scope-registry.ts` — 路径受限门禁的**扫描范围**读取与通配展开（#733 计划项 3.2.1）：未登记 / 范围解析为空一律抛错（未登记即红）。
- `lib/config-matrix-gate.ts` — 配置平行事实源「字段覆盖矩阵」门禁的**编排层**（issue #471）：`runConfigMatrix({ root })` 对两包真实 src 执行全部矩阵断言并返回 `{ pass, problems, lines }`，不自行 console / exit（退出码由调用方的 `gate/contract-check.ts` 追加段与负向自测决定）；文件树以 root 参数化，故负向用例可对 mkdtemp 副本注入后复用同一逻辑。提取器与矩阵语义在 `lib/config-matrix-lib.ts`。
...
- `test/verify-host-seams.test.ts` — 宿主接缝门禁自测：T1-T7 收集向量（条件两支、空值合并右操作数、数组对象内路径、return、MIME 与句中 GET 与空串排除）、R1-R4 锚定向（位置允许、根 shared 段、basename 允许集、装配点、分组 disposition、receiver 不限名）、fail-closed 唯二出口与禁 --package 例、真实仓库锚（收敛后 exit 0，观察表含 triple 与镜像对）。
...
- `data/dir-imports-baseline.json` — 目录导入门禁的单调基线（#690 / #733 A）：包键集 = 该闸 cli 调用点并集、且每个调用文件各自覆盖全集（#843 D15，漂移即红），结构型计数由 `--write-baseline` 登记，`quality` 段存质量型**证据集合**（边 `from|to|kind`、环签名、未覆盖源文件）。#767 B0 切片 3a 另加 `crossDomainValueEdges` / `rootIndexImports` / `clientServerImports` / `unitImportFaceViolations` 四键）。**首次登记分两级**：包级（旧基线无该包，或旧基线是数字口径）只走台账通道；**证据类首次登记**（基线里没有该键 = 本次新增了一类证据）按当前事实写入并逐类提示；类内新增证据仍一律不写入、判红（放宽须走台账）。
...
- `data/ci-face-registry.json` — `packages/` 之外每个 tracked 文件的 CI 归属登记（#742 阶段 2.3；#843 L5 起带消费方；faces 语义 #875 H9 改）：`faces` 可缺省——写了就必须非空且是 ci.yml 的真实面名（`global` / 包名），**空数组=显式豁免的通道已删除**；缺省表示「该条命中的文件都不需要任何 CI 面」，而这由 `test/ci-face-coverage.test.ts` 的三条规则从对象特征推导并核对：N1 ci.yml 无 glob 命中、N2 无 `packages/**` 代码 import 它、N3 `consumers` 不含包内路径（N1 单独即等价于「无面」，实现里是判定循环的入口条件，保留在判定本体中是为语义完整与载体自证可触达）。**声明了面就仍然必须写 `why`**，无面条目则不得写（理由由 N1/N2/N3 给出）。N2 的说明符提取是文本级，已实测的八类漏报：动态拼接 specifier、模板字面量 import、`#fragment`、`?query` 后缀、`createRequire` 的 `req(...)`、`import.meta.resolve`、`readFileSync(new URL(...))`、`.js` 说明符指向真实 `.ts`；能检出的含静态 `from`、`import("字面量")`（含编译期条件导入）、`require("字面量")`、副作用导入 `import "x"`。**N2/N3 在当前真实数据上检出恒为 0/293，真实防护从未被端到端验证过，今天的价值靠载体自证维持。**另有 `consumers`（仓内直接读取方，路径必须存在且文件文本字面命中 source；没有直接读取方时改声明 `consumerGap`，`kind` ∈ none / external / indirect，两者排他）。`test/ci-face-coverage.test.ts` 用真实 `git ls-files` 与 ci.yml 的 filters 双向核对（未登记、悬空条目、面集合**等值**（漏面与多面都判红）、死 glob、消费方不实、无面资格推导失败六类都判红）。

- `data/threshold-registry.json` — 阈值事实源的**声明表**（#843 D5）：`kind` = value（数字标量，`weaken` 指明哪个方向是放宽）/ boolean（开关，`weakenValue` = 等于它就等于放宽）/ baseline（逐包回落锚点，`anchorFields` 是回退链，读法与 `gate/observe-check.mjs` 的 `fixedCovered ?? baselineCovered` 一致）/ existence（集合存在性，`universe` 从 `packages/` 目录派生、`exemptFrom` 读 `mutation-topology.json` 的 `$noMutationPackages`）；`notAGate[]` 逐条说明「为什么它不是阈值事实源」。未登记的数据文件、幽灵声明、幽灵判据三类都判红；`kind=existence` 的豁免登记在 `data/gate-exemptions.json`（gate=`threshold-registry`），**按判据分开**：path=`<guard.path>.<包名>#membership` 只豁免「包在表里」、`#anchor` 只豁免「有回落锚点」，value 守卫的删键用 `<被移除的叶子路径>#removal`；认不出的键与已无缺口的键都判红（反向腐烂）。本表**不含任何自授权通道**：`retired` / `contractApprovals` 两个键已于 #875 H11 整体删除，重现即判红。
