...
- `maintenance/equiv-check.ts` — 等价重构的「机制性等价性检查器」（#843 M10，即 DEVELOPMENT §4.1 验证三件套第 2 件）：把基线 ref 与改动 ref 两侧源码各解一次 AST（复用 `lib/config-matrix-lib.ts` 的 `parseTs`，不另写解析器、不引新依赖），抽取**字面量 / 非计算对象键 / 正则 / `process.exit(n)` 与 `process.exitCode = n`** 的多重集做差集，外加 §4.1 点名的两个专项（**数字字面量整段消失**防丢上界阈值、**新增函数零引用**防抽了不用）；差异只允许「空」或抽函数必然产生的结构回声，后者进登记表——按（文件 × 类别）分组、组内逐条枚举（值, 计数差，不接受通配符 / 前缀 / 范围）+ 每组一条理由，**未登记差异 exit 1**，登记项本次没命中同样 exit 1（反向腐烂），无差异 exit 0，**ref 不存在 / 文件读不到 / 解析失败 / 登记表本身坏了一律 exit 2**（坏表不退化成「未登记」，三态比 `scan-actions-concurrency.mjs` 的 0/2 补齐了 1）。**覆盖面边界（常驻 `--help` 与每次输出、`--json` 走 `boundary` 字段）：本工具不构成等价性证明**——只覆盖上述多重集差集与两个专项，控制流等价性不在面内（分支顺序对调、条件取反、状态少复位一次都照样报 0 差异），须由 §4.1 第 3 件的差分对拍证明。自测见 `test/equiv-check.test.ts`（含「把控制流回归种回去时工具仍报 0」这条防误用例）。维护者 / 本地工具，需 git 完整历史（要比对历史 ref），故**不进 CI**（进 CI 属 `.github/` 红线段，且维护工具不进 CI 属 AGENTS.md 明写的设计）。
