...
- `data/ci-face-registry.json` — `packages/` 之外每个 tracked 文件的 CI 归属登记（#742 阶段 2.3；#843 L5 起带消费方；faces 语义 #875 H9 改）：`faces` 可缺省——写了就必须非空且是 ci.yml 的真实面名（`global` / 包名），**空数组=显式豁免的通道已删除**；缺省表示「该条命中的文件都不需要任何 CI 面」，而这由 `test/ci-face-coverage.test.ts` 的三条规则从对象特征推导并核对：N1 ci.yml 无 glob 命中、N2 无 `packages/**` 代码 import 它、N3 `consumers` 不含包内路径。声明了面才需要 `why`；另有 `consumers`（仓内直接读取方，路径必须存在且文件文本字面命中 source；没有直接读取方时改声明 `consumerGap`，`kind` ∈ none / external / indirect，两者排他）。`test/ci-face-coverage.test.ts` 用真实 `git ls-files` 与 ci.yml 的 filters 双向核对（未登记、悬空条目、面集合**等值**（漏面与多面都判红）、死 glob、消费方不实、无面资格推导失败六类都判红）。
