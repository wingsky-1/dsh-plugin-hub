...
- `gate/red-line-approval.mjs` — 红线路径改动的「人工批准」判据（#843 M1 的代码侧落点）：PR 的 changed files 命中红线面（常量数组 `RED_LINE_PATTERNS`，当前 `.github/**` 与 `scripts/gate/**`）且 labels 不含 `approved` 即判红；纯函数 `judgeRedLine` + CLI（输入缺失/不可解析 exit 2、判红 exit 1、放行 exit 0）。执行点 = `ci.yml` 的 `Build / Red-line approval` job（仅 PR），该 job 已挂进 repo-gate 的 `needs`。**本文件（`scripts/README.md`）不在红线面内**——两条 glob 都不命中它，改它不需要 `approved` 标签。
