...
- `gate/verify-dir-imports.mjs` — 目录 `interface.ts` 门面静态检查 + 依赖图尺子（#664 D10 / #670 C2 / #690 S0）：跨模块引用只许走目标模块的 `interface.ts`，单调基线（`data/dir-imports-baseline.json`）存**证据集合**，放宽须登记到 `data/gate-exemptions.json`（gate=`verify-dir-imports`，path=`<包名>:<证据项>`，与另两闸同一套台账）；变异拓扑 `data/mutation-topology.json` 缺失 / 解析失败 / 内容非对象一律 fail-closed（#773 R3），`--write-baseline` 同样中止。#767 B0 切片 3a 起另有三类证据（都是**只许缩小**的集合，终态为空）：I2① 域间值边 `crossDomainValueEdges`（叶子模块粒度、目标非共享层）、I2④ `rootIndexImports`（域内文件值引 src 根 `index.ts`）、§5.3 `clientServerImports`（`src/client/**` 不得 import `src/server/**`，**独立一遍扫描**，故各包既有结构型计数零位移）。
...
- `data/dir-imports-baseline.json` — 目录导入门禁的单调基线（#690 / #733 A）：结构型计数由 `--write-baseline` 登记，`quality` 段存质量型**证据集合**（边 `from|to|kind`、环签名、未覆盖源文件；#767 B0 切片 3a 另加 `crossDomainValueEdges` / `rootIndexImports` / `clientServerImports` 三键）。**证据类首次登记**（基线里没有该键 = 本次新增了一类证据）按当前事实写入并逐类提示；类内新增证据仍一律不写入、判红。
