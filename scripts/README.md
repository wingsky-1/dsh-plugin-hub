...
- `test/threshold-monotonic.test.ts` — 阈值声明表与单调性自测（#722 / #843 D5）：coverage.thresholds 提取、降线判红、缺块 fail-closed，外加声明表三类判红（未登记数据文件 / 幽灵声明 / 幽灵判据）与 value / boolean / baseline / existence 四类 kind 的放宽判红与收紧放行（fixture 自带一份最小声明表，与真值快照各跑一遍）。
