...
- `data/dir-imports-baseline.json` — 目录导入门禁的单调基线（#690 / #733 A）：结构型计数由 `--write-baseline` 登记，`quality` 段存质量型**证据集合**（边 `from|to|kind`、环签名、未覆盖源文件；#767 B0 切片 3a / 3b 另加 `crossDomainValueEdges` / `rootIndexImports` / `clientServerImports` / `unitImportFaceViolations` 四键）。**首次登记分两级**：包级（旧基线无该包，或旧基线是数字口径）与**证据类级**（基线里没有该键 = 本次新增了一类证据）——两级都按当前事实写入并逐条提示「须在 PR 内确认」；键一旦存在，类**内**新增证据仍一律不写入、判红（放宽须走台账）。
