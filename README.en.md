...
- The official type-layer catalog in `pnpm-workspace.yaml` is the single version source; each
  package's peerDependencies is an exact-version projection generated by `pnpm catalog:sync-peers`.
  All plugins are pinned to `dsh 0.1.7-rc.2`; **the DSH CLI and every linked/registry plugin must match this exact target**; other DSH versions are unsupported
- npm/pnpm will surface a peer mismatch if your dsh version does not match. Publish/install
  plugins whose exact peer version matches the target DSH first, then switch the DSH CLI;
  do not use `allow-version` to bypass an incompatible runtime
