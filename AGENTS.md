...
| 收尾     | `pnpm gate:full`                          | 全仓对象面（同 `gate:pr`，另加「豁免到期台账」收集；`--with-coverage` 再补 cov / crap）：全仓 build/test/typecheck + 全仓产物闸 + 全部静态闸。**≠ 夜间班次口径**——夜间 `observe.yml` 另含覆盖率与全量变异、且不跑全仓 test/typecheck（全仓 build 在夜间两个 job 内各有一处：quality 一次 + mutation-shards 每个变异段一次，当前 33 段）；需要覆盖率时用 `pnpm gate:full --with-coverage`。改过构建链、包结构或发版前跑一遍                                                                                                                                                                    |
