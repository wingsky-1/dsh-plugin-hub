 ...
 *
 * 为什么需要本文件：阶段四目录化曾把组合根移入 src/apply/（#768 D13 起包根 src/index.ts
 * 已落地，为 apply/index.ts 的显式具名薄转发，tsc 随之正常产出 lib/index.js/d.ts）；
 * 但包的发布面与全部外部
 ...
 * 注：本步骤覆盖 tsc 对 src/index.ts 的 emit 产物，但覆盖前后同靶（apply/index.ts
 * 具名薄转发），语义等价，build 前后导出面快照双 PASS 实证；
 * esbuild 在 bundle-host 中沿转发递归内联，最终 lib/index.js 与重构前产物内容等价。
 *
 * 同时把 src/server/adapters/*.{mjs,d.mts} 拷贝到 lib/server/adapters/（与 tsc
 * 产物的相对引用结构一致）：.mjs/.d.mts 不被 tsc emit，而 lib/apply/index.js 的
 * `export * from "../server/adapters/xxx.mjs"` 与 index.d.ts 的类型解析都需要它们在位。
 */
...
console.log(
  "[prepare-lib-entry] 适配器产物归位 lib/server/adapters/ 与入口转发 lib/index.js|d.ts 完成",
);
