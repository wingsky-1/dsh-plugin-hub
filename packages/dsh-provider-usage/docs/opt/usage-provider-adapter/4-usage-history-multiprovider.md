...
- 启动扫描：若存在旧 `history.json`（在 `dsh-provider-usage/` 目录）：
...
### 割接矩阵（重点：历史 opencode-usage（旧版插件）数据割接）

「割接」指：现有用户（发布版 main 0.1.9 及此前，单文件 v1；M3a 分支单文件 v2）升级到
多文件 v3 时，**已有旧版 opencode-usage 插件的历史采样点必须平滑过渡、不丢失、三窗口列语义延续**。
