...
用户把复制的一句话发到会话，即触发本流程。指令示例：

> 请为提供商 \<provider\> 创建用量统计适配器：自行查找其用量接口与鉴权方式，
> 自主设计适配器方案（id/展示名/窗口字段），先给我审核方案，确认后生成 .mjs
> 文件、告诉保存路径并引导我在「用量统计」设置页添加适配器。

收到后按以下流程执行（默认全程自主，只保留审核点）：

1. **盘点已知**（0 提问）：从用户当前会话/模型配置/已有文件的上下文收集：provider 名、
   baseUrl、apiKey、已有的接口线索。
2. **自主调研**（0 提问）：查该 provider 的官方 API 文档或接口约定（用量/额度/配额
   端点、鉴权头、返回结构）。查到即用；查不到但**能从已有信息合理推导**（如
   `{baseUrl}/v1/usage`、Bearer 鉴权）也直接用并标注「推断」。
3. **产出决策审核卡**（给用户看，等确认）：用简短清单列出全部实质决策，例如：

   ```text
   ── 适配器方案（请审核）────────────────
   · provider 认领：my-relay（来自你的模型配置）
   · 接口：GET https://relay.example.com/v1/usage（官方文档 → 链接）
