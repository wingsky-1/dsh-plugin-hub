...
| 区 | 内容 |
| --- | --- |
| 提供商（主区） | 与 dsh 模型配置页提供商列表精确一致（`modelProviders` 同源）；每个提供商展开后展示**适配器候选行**，每行**独立开关**（唯一启用——开启一个自动关掉上一个；关闭当前启用者 = 停用该 provider 的用量取数，不涉及模型配置页的提供商可用性）。`[+ 添加适配器]` 内嵌于对应提供商：仅输入文件路径 → `POST /adapters/inspect` 检测回显导出信息（id/label/providers/version，不注册）→ 确认后 `POST /adapters/add`（仅 file，身份以导出为准）热注册。候选/启用态指向不在模型配置中的 provider 收进尾部独立分组「自定义提供商」（避免静默消失，不计入主列表数量）。**是「用户自定义 js」的可视化配置落点** |
