...
演示如何为 `@wingsky-1/dsh-provider-usage` 接入一个自定义中转站：
...
```sh
# 候选已注册且启用
curl -s http://127.0.0.1:3080/api/dsh-provider-usage/adapters.json

# 会话模型选到 my-relay provider 后，浮窗出现「中转 额度 xx%」；
# 或直接验证取数链路（provider 参数）：
curl -s "http://127.0.0.1:3080/api/dsh-provider-usage/stats?provider=my-relay"
```
...
```sh
# 切到本示例适配器
curl -s -X POST http://127.0.0.1:3080/api/dsh-provider-usage/adapters/select \
