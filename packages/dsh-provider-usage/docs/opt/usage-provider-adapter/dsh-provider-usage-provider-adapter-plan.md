# dsh-provider-usage 演进方案（主文件）：按 Provider 适配 + 用户自定义 JS 注入

> 状态：**M1 适配器框架 + M2 用户 JS 注入已实施（分支 `feature/provider-adapter`），
> 本文件为剩余阶段 R1~R3+M3b 的设计定稿与跟进跟踪**
> 涉及插件：`github/dsh-plugin-hub/packages/dsh-provider-usage`（@wingsky-1/dsh-provider-usage）
>
...
- **方案**：
  - **切换**：`POST /api/dsh-provider-usage/adapters/select`（body `{ provider, adapterId }`，
...
> 路径：`github/dsh-plugin-hub/packages/dsh-provider-usage/docs/opt/usage-provider-adapter/`
