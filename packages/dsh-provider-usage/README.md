...
```sh
dsh plugin --profile web add @wingsky-1/dsh-provider-usage@<版本号>
```
...
| `persistFile` | 无（默认历史根 `<DSH_HOME>/dsh-provider-usage/`，多文件桶存于其 `history/` 子目录） | 历史根目录覆盖（其目录为历史根） |
...
- 运行时切换：设置面板（M3b）或 `POST /api/dsh-provider-usage/adapters/select`
...
- 管理端点信息面最小披露：`adapters.json` / `health` 中用户适配器文件只显示
  文件名（不外泄绝对路径），`health.history` 不含数据根目录位置
