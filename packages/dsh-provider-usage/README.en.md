...
- **State and recovery**: a per-`period + report key` ledger persists attempts, `nextRetryAt`, terminal state/reason, per-attempt observations, and cumulative usage. Restart recovery preserves the budget and terminal state. A failed fifth retry or any non-retryable class stops automatic calls. Manual **Regenerate** (force) alone starts a new cycle, resets the count, and snapshots current configuration; a failed force still does not advance lastRun.
