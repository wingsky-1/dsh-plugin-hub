/**
 * dsh-provider-usage — 客户端文案字典（复用官方 dsh-client-locale）。
 *
 * 双语平衡：`zh` 为 key 源；`en` 必须覆盖全部 key（编译期锁平衡）。
 * 动态数据用 `{name}` 占位模板，渲染期由 t 插值。
 * 不进字典：引导指令（发给 Agent 的 prompt 数据）、路径 placeholder 示例、
 * console 日志、宿主端渲染的 capsuleHtml（官方原则：数据不翻译）。
 */

/** 简体中文字典（key 源）。 */
export const zh = {
  // 相对时间（胶囊/面板）
  justNow: "刚刚",
  minutesAgo: "{n} 分钟前",
  hoursAgo: "{n} 小时前",
  daysAgo: "{n} 天前",
  // 胶囊 title
  pillNotConfigured: "未配置适配器，点击查看接入引导",
  pillFetchFail: "用量获取失败：{msg}",
  pillBusy: "取数进行中，稍候自动刷新",
  pillStale: "数据陈旧",
  pillCached: "缓存",
  pillFresh: "实时",
  pillUpdatedAt: "更新于 {t}",
  providerUnknown: "提供商未识别",
  // 悬浮面板
  panelTitle: "{provider} 用量",
  refresh: "刷新",
  hintPrefix: "提示：",
  loading: "加载中…",
  ariaPill: "用量统计",
  settingsTab: "用量统计",
  // 设置页：六分段器 tab（#940 终稿六标签；history 紧随 report）
  settingsNavLabel: "用量统计设置分区",
  pageTabTrend: "使用趋势",
  pageTabReport: "用量报告",
  pageTabHistory: "历史报告",
  pageTabUsage: "用量可视化",
  pageTabProviders: "适配器",
  pageTabFloat: "悬浮窗",
  // 设置页共用头（#940 终稿 settings-top；pills 走 live 数据，见 settings/index.tsx）
  settingsKicker: "Settings · Provider Usage",
  settingsTitle: "用量与报告",
  settingsProvidersOk: "{n} 个 Provider · 状态正常",
  settingsProvidersStale: "{n} 个 Provider · 部分陈旧",
  // 错误表（errorMessage）
  errNoApiKey: "未找到 API Key（走 {p}_API_KEY 环境变量或 .credentials.yaml）。",
  errUnauthorized: "API Key 无效或已过期（401/403）。",
  errTimeout: "请求超时，请重试。",
  errNetwork: "网络错误，请检查连接后重试。",
  errBadData: "数据格式异常，请检查服务商接口或适配器返回。",
  errAdapterLoadFailed: "自定义适配器加载失败，请检查文件路径与契约（设置页可见错误明细）。",
  errBusy: "上一次取数仍在进行，稍候自动刷新。",
  errNoEnabledAdapter: "该提供商有候选适配器但均未启用，请在设置面板「用量统计」启用。",
  errNoAdapter: "该提供商未配置适配器，请在设置面板「用量统计」添加。",
  errHttpStatus: "服务返回 {code}，请检查中转站状态。",
  errGeneric: "用量获取失败：{code}。",
  noDataShort: "暂无数据",
  // 无启用适配器引导（胶囊面板）
  noAdapterTitle: "该提供商暂无启用的适配器。",
  noAdapterHint:
    "在设置面板「用量统计」的适配器管理中启用一个候选适配器；也可复制下方引导指令，让 Agent 帮你接入用量数据源。",
  copyGuide: "复制引导指令",
  copied: "已复制 ✓",
  copyFail: "复制失败",
  // 设置页：状态/位置
  statusFresh: "实时",
  statusCached: "缓存",
  statusStale: "陈旧（已降级）",
  statusUnconfigured: "未配置",
  posTopRight: "右上",
  posTopLeft: "左上",
  posBottomRight: "右下",
  posBottomLeft: "左下",
  uiTitle: "胶囊位置",
  uiAnchor: "锚点：",
  offsetX: "水平偏移",
  offsetY: "垂直偏移",
  panelOffsetY: "面板间距",
  zIndexBase: "层级基准",
  savingNow: "保存中…",
  save: "保存",
  uiSavedOk: "已保存——胶囊位置即时生效（所有设备）",
  uiSaveFail: "保存失败：{msg}",
  // 设置页：用量可视化
  usageTitle: "用量可视化",
  usageToday: "今日概览（/trend 日面；调用失败不影响下方状态点）",
  usageTodayTokens: "今日 Token",
  usageDonutIO: "Token 构成（最新有数日）",
  usageIOLabelInput: "输入",
  usageIOLabelOutput: "输出",
  usageIOLabelCache: "缓存",
  usageDonutModels: "模型分担（窗口 byProvider）",
  usageActiveLabel: "活跃天数",
  usageHeat: "用量热力图",
  usageHeatDays: "{n} 天",
  usageHeatNote: "色阶按窗口最大值相对分档",
  usageLoading: "概览加载中…",
  usageLoadFail: "概览加载失败：{msg}",
  noProviders: "暂无启用的 provider",
  noData: "暂无数据",
  updatedAt: "更新于 {t}",
  // 设置页：适配器管理
  badgeEnabled: "已启用：{name}",
  badgeOff: "未启用适配器",
  adapterBuiltin: "内置",
  adapterCustom: "自定义",
  switchOffTitle: "停用该适配器（仅影响用量取数）",
  switchOnTitle: "启用该适配器",
  lastError: "最近一次错误：{msg}",
  inspectKName: "适配器名",
  inspectKLabel: "展示名",
  inspectKProviders: "归属提供商",
  inspectKVersion: "契约版本",
  filePath: "文件路径",
  filePathOnly: "（唯一输入——name/展示名/归属提供商从文件导出自动读取）",
  pathKeyHint: "仅接受本地 .mjs · 密钥不进端",
  detecting: "检测中…",
  detectFile: "检测文件",
  detectFirst: "请先通过「检测文件」后再确认添加",
  adding: "添加中…",
  confirmAdd: "确认添加",
  cancel: "取消",
  inspectFail: "检测失败",
  addFail: "添加失败",
  noCandidates:
    "该提供商暂无候选适配器——可通过下方 [+ 添加适配器] 注入本地适配器文件；也可复制引导指令，让 Agent 帮你创建适配器。",
  collapse: "收起",
  addAdapter: "+ 添加适配器",
  // 设置页：提供商列表区
  provTitle: "提供商",
  provSub: "列表默认折叠 · 密钥不进端",
  provListHint:
    "提供商列表与模型配置页保持一致（共 {n} 个）；展开某个提供商以切换适配器开关或添加新适配器。",
  fileLoadFail: "用户文件 {f} 加载失败：{msg}",
  noProvHint:
    "未发现已配置的提供商（adapters.json 不可达或模型配置页尚未配置提供商）。可复制引导指令，让 Agent 帮你接入数据源并创建适配器。",
  customProvTitle: "自定义提供商",
  customProvHint:
    "以下 providers 未出现在 dsh 模型配置页（模型暂选不到该提供商，先登记适配器；在模型页补建后即可选用），不计入上方数量。",
  // 设置页：使用趋势
  trendTitle: "使用趋势",
  trendGranularity: "粒度",
  trendGranDay: "日",
  trendGranWeek: "周",
  trendGranMonth: "月",
  trendMetricLabel: "指标",
  trendMetricTotal: "总 tokens",
  trendMetricInput: "输入",
  trendMetricOutput: "输出",
  trendMetricCacheRead: "缓存读",
  trendMetricCacheWrite: "缓存写",
  trendMetricCalls: "调用次数",
  trendAdapterLabel: "适配器",
  trendAdapterAll: "全部适配器",
  trendByModel: "按模型拆分",
  trendCardTotal: "窗口总量",
  trendCardCalls: "调用次数",
  trendCardPeak: "峰值桶",
  trendCaliberNote: "含进行中桶 · 尾桶未满",
  trendCardTop: "Top 适配器",
  trendCardTopDir: "Top 目录", // 目录面 Top 汇总卡标签（与 trendCardTop 同风格对称）
  trendFetchFail: "趋势数据获取失败",
  trendEmptyTitle: "暂无趋势数据",
  trendEmptyHint: "会话产生用量后，这里会展示近期的使用趋势。",
  // 设置页：使用趋势（视图改版）
  trendRangeLabel: "范围",
  trendRangeDayUnit: "{n} 天",
  trendRangeWeekUnit: "{n} 周",
  trendRangeMonthUnit: "{n} 个月",
  trendViewLabel: "视图",
  trendViewBar: "柱状",
  trendViewArea: "面积",
  trendPartialOngoing: "进行中",
  trendPartialEdge: "部分桶",
  trendNoData: "无数据",
  trendTipSum: "合计",
  trendPrevIncomplete: "环比基准不完整",
  trendHiddenParts: "已隐藏 {k} 段（卡为全段口径）",
  trendLegendToggleHint: "点击图例可显隐对应段",
  trendPeakDay: "{day}",
  trendPeakWeek: "{day} 当周",
  trendPeakMonth: "{y} 年 {m} 月",
  trendCardAvg: "日均",
  trendViewVisible: "可见段",
  // 设置页：使用趋势目录维度
  trendDirLabel: "目录",
  trendDirAll: "全部目录",
  trendDirUnidentified: "未识别",
  trendDirUnidentifiedNote: "无目录信息的会话",
  // 设置页：用量报告
  reportTitle: "用量报告",
  reportPeriodDaily: "日报",
  reportPeriodWeekly: "周报",
  reportPeriodMonthly: "月报",
  reportEnabled: "启用",
  reportTime: "触发时间",
  reportWeekStartsOn: "周起点",
  reportWeekMonday: "周一",
  reportWeekSunday: "周日",
  reportDayOfMonth: "月内第几天",
  reportProvider: "模型提供商",
  reportProviderDefault: "跟随默认",
  reportModel: "模型",
  reportModelHint: "留空跟随默认",
  reportModelDefault: "跟随默认（首个已注册模型）",
  reportModelKept: "{v}（当前配置值，不在列表中）",
  reportModelFallback: "模型列表拉取失败或为空，已降级为手动填写。",
  reportReasoningEffort: "思考等级",
  reportReasoningUnset: "未配置（跟随 DSH 默认）",
  reportReasoningDefault: "DSH 默认",
  reportReasoningStaleOption: "{v}（已保存，当前模型未声明）",
  reportReasoningUnavailable: "当前模型未提供 reasoning effort 能力。",
  reportReasoningCapabilityError: "当前模型的 reasoning effort 能力暂不可用；现有配置已保留。",
  reportReasoningStale: "已保存的思考等级不在当前模型能力中；生成时会安全失败。",
  // 报告目录范围多选
  reportDirectories: "目录范围",
  reportDirectoriesAll: "全部目录",
  reportDirectoriesSelectAll: "全选",
  reportDirectoriesClear: "清空",
  reportDirectoriesEmpty: "暂无目录数据——会话产生用量后，这里会列出可选目录。",
  reportDirectoriesHintAll: "统计全部目录（默认口径）。",
  reportDirectoriesHintScoped: "报告只统计所选目录的目录分布（其余维度保持全量）。",
  reportPromptReset: "恢复默认",
  reportPromptHint: "分段编辑，整体保存；红线约束默认折叠。",
  reportNoData: "当期无用量数据，未生成报告",
  reportHeroTotal: "tokens",
  reportHeroCalls: "对话次数",
  reportHeroActive: "活跃天数",
  reportHeroStreak: "最长连续",
  reportHeroPeak: "峰值日",
  reportRatioFlat: "持平",
  reportRatioPct: "{n}%",
  reportPrompt: "提示词模板",
  reportPush: "生成后推送",
  reportSave: "保存配置",
  reportSaved: "已保存",
  reportSaveFail: "保存失败：{msg}",
  reportGenerate: "立即生成",
  reportGenerating: "生成中…",
  reportGenerateFail: "生成失败：{msg}",
  // 轮询超时正向提示（任务可能仍在后台生成，不报失败）
  reportStillGenerating: "仍在后台生成，稍后刷新列表查看",
  // 幂等复用提示 + 强制重新生成勾选
  reportReused: "该窗口已有报告，已复用现有结果（勾选「重新生成」可覆盖）",
  reportForceRegen: "重新生成（覆盖）",
  reportHistory: "报告历史",
  reportEmpty: "尚未生成过报告——配置周期或立即手动生成",
  reportFilterEmpty: "当前筛选下暂无报告——切换筛选或全部查看",
  reportFetchFail: "报告数据获取失败",
  reportOk: "成功",
  reportFailed: "失败",
  reportCollapse: "收起",
  reportDetailTokens: "生成消耗：{n} tokens",
  reportPeriodSelect: "选择周期",
  // #940 P0：折叠 + 结构化提示词（第一批起报告四区＋历史独立页）
  reportSectionSchedule: "调度",
  reportSectionRouting: "路由与范围",
  reportSectionPrompts: "报表提示词",
  reportSectionGenerate: "手动生成",
  reportSectionHistory: "历史报告",
  reportExpandAll: "全部展开",
  reportCollapseAll: "全部收起",
  reportUnsaved: "未保存",
  reportSectionOff: "关闭",
  reportSummaryNDirs: "{n} 个目录",
  reportSummaryNReports: "{n}份报告",
  reportGoalLabel: "任务目标",
  reportStatsLabel: "统计数据",
  reportStructureLabel: "撰写结构",
  reportConstraintsLabel: "硬性约束",
  reportConstraintCount: "{n}条约束",
  reportBudgetLabel: "字数预算",
  reportPromptWordUnit: "字",
  reportStatsVarNote: "{stats} 为统计 JSON 注入点（只读，不可改名）",
  reportPromptRawHint: "模板非标准结构，已切换整体编辑，文本无损",
  // #940 第一批：历史独立页 + 生成脚注 + 短状态（终稿验收项）
  historySub: "展开查看明细 · 按需加载",
  reportFilterStatus: "状态",
  reportFilterAll: "全部",
  reportFilterNoData: "空窗口",
  reportLoadMore: "加载更多",
  reportForceShort: "强制覆盖",
  reportNoForceShort: "不强制覆盖",
  reportGenIdempotentHint: "窗口已有成功报告时默认幂等复用；勾选强制才会覆盖重跑。",
  reportRangeVarNote: "{range} 为窗口范围注入点（只读，不可改名）",
  // T4 月报弱模型策略：模板不加降级正文，配置页 footnote + 重试指引
  reportWeakModelNote: "月报篇幅长，弱模型可能降级或失败；失败可重试，或更换模型后重新生成。",
  reportRetryHint: "生成失败可重试；如需覆盖已有报告请勾选「重新生成」。",
  reportRetryRunning: "报告 attempt 正在执行；以下为当前周期累计成本。",
  reportRetryBusy: "该报告窗口已有 attempt 正在执行；以下为当前周期累计成本。",
  reportRetryDeferred: "当前 attempt 未完成，已安排后续自动重试。",
  reportRetryTerminal: "自动重试已终止：{code}（{kind}）。",
  reportRetryNextAt: "下次自动重试：{at}",
  reportRetryAttempt: "当前报告 attempt {attempt}；自动重试上限 {maxAttempts} 次。",
  reportRetryInputTokens: "累计输入 token：{value}",
  reportRetryOutputTokens: "累计输出 token：{value}",
  reportRetryReasoningTokens: "累计推理 token：{value}",
  reportRetryTotalTokens: "累计总 token：{value}",
  reportRetryCacheTokens: "累计缓存 token：读取 {read}，写入 {write}",
  reportRetryDuration: "累计耗时：{value} ms",
  reportRetryUnknown: "未提供",
  // PM1 hero 日报高峰钟点行
  reportHeroPeakHour: "高峰钟点",
  reportPeakHourValue: "{n} 点",
  // D1 未见目标行的用户可见反馈（禁止静默吞键）
  reportGeneratedNotVisible: "报告已生成，但列表中暂未见到，可重新加载后查看。",
  reportReloadList: "重新加载",
} as const;

/** 字典 key 并集（LocaleNamespaceMap 声明合并用）。 */
export type ProviderUsageLocaleKey = keyof typeof zh;

/** 英文词典：必须与 zh key 完整对齐。 */
export const en: Record<ProviderUsageLocaleKey, string> = {
  justNow: "just now",
  minutesAgo: "{n} min ago",
  hoursAgo: "{n} h ago",
  daysAgo: "{n} d ago",
  pillNotConfigured: "No adapter configured — click for onboarding",
  pillFetchFail: "Usage fetch failed: {msg}",
  pillBusy: "Fetch in progress, refreshing shortly",
  pillStale: "stale data",
  pillCached: "cached",
  pillFresh: "live",
  pillUpdatedAt: "updated {t}",
  providerUnknown: "provider unidentified",
  panelTitle: "{provider} usage",
  refresh: "Refresh",
  hintPrefix: "Hint: ",
  loading: "Loading…",
  ariaPill: "Usage statistics",
  settingsTab: "Usage statistics",
  // Settings page: six-tab segmenter
  settingsNavLabel: "Usage statistics sections",
  pageTabTrend: "Trends",
  pageTabReport: "Reports",
  pageTabHistory: "History",
  pageTabUsage: "Usage",
  pageTabProviders: "Adapters",
  pageTabFloat: "Float",
  settingsKicker: "Settings · Provider Usage",
  settingsTitle: "Usage reports",
  settingsProvidersOk: "{n} providers · healthy",
  settingsProvidersStale: "{n} providers · partially stale",
  errNoApiKey: "API key not found (set the {p}_API_KEY environment variable or .credentials.yaml).",
  errUnauthorized: "API key invalid or expired (401/403).",
  errTimeout: "Request timed out, please retry.",
  errNetwork: "Network error, check the connection and retry.",
  errBadData: "Malformed data — check the provider API or adapter output.",
  errAdapterLoadFailed:
    "Custom adapter failed to load — check the file path and contract (details on the settings page).",
  errBusy: "Previous fetch still in progress, refreshing shortly.",
  errNoEnabledAdapter:
    "This provider has candidate adapters but none enabled — enable one under Usage statistics.",
  errNoAdapter: "No adapter configured for this provider — add one under Usage statistics.",
  errHttpStatus: "Service returned {code} — check the relay status.",
  errGeneric: "Usage fetch failed: {code}.",
  noDataShort: "no data",
  noAdapterTitle: "No enabled adapter for this provider.",
  noAdapterHint:
    "Enable a candidate adapter under Usage statistics, or copy the guide prompt below and let an agent wire up the data source.",
  copyGuide: "Copy guide prompt",
  copied: "Copied ✓",
  copyFail: "Copy failed",
  statusFresh: "live",
  statusCached: "cached",
  statusStale: "stale (degraded)",
  statusUnconfigured: "unconfigured",
  posTopRight: "Top right",
  posTopLeft: "Top left",
  posBottomRight: "Bottom right",
  posBottomLeft: "Bottom left",
  uiTitle: "Pill placement",
  uiAnchor: "Anchor: ",
  offsetX: "Horizontal offset",
  offsetY: "Vertical offset",
  panelOffsetY: "Panel gap",
  zIndexBase: "Z-index base",
  savingNow: "Saving…",
  save: "Save",
  uiSavedOk: "Saved — pill placement takes effect immediately (all devices)",
  uiSaveFail: "Save failed: {msg}",
  usageTitle: "Usage visualization",
  usageToday: "Today overview (/trend daily; fetch failure does not affect status dots below)",
  usageTodayTokens: "Today Tokens",
  usageDonutIO: "Token mix (latest day with data)",
  usageIOLabelInput: "Input",
  usageIOLabelOutput: "Output",
  usageIOLabelCache: "Cache",
  usageDonutModels: "Model share (window byProvider)",
  usageActiveLabel: "Active days",
  usageHeat: "Usage heatmap",
  usageHeatDays: "{n}d",
  usageHeatNote: "Shades are relative to the window maximum",
  usageLoading: "Loading overview…",
  usageLoadFail: "Overview load failed: {msg}",
  noProviders: "No enabled provider",
  noData: "No data",
  updatedAt: "updated {t}",
  badgeEnabled: "Enabled: {name}",
  badgeOff: "No adapter enabled",
  adapterBuiltin: "built-in",
  adapterCustom: "custom",
  switchOffTitle: "Disable this adapter (usage fetching only)",
  switchOnTitle: "Enable this adapter",
  lastError: "Last error: {msg}",
  inspectKName: "Adapter name",
  inspectKLabel: "Display name",
  inspectKProviders: "Providers",
  inspectKVersion: "Contract version",
  filePath: "File path",
  filePathOnly: " (the only input — name/display name/providers are read from the file exports)",
  pathKeyHint: "Local .mjs only · keys never reach the client",
  detecting: "Detecting…",
  detectFile: "Detect file",
  detectFirst: 'Run "Detect file" before confirming',
  adding: "Adding…",
  confirmAdd: "Confirm",
  cancel: "Cancel",
  inspectFail: "Detection failed",
  addFail: "Add failed",
  noCandidates:
    "No candidate adapters for this provider — inject a local adapter file via [+ Add adapter] below, or copy the guide prompt and let an agent create one.",
  collapse: "Collapse",
  addAdapter: "+ Add adapter",
  provTitle: "Providers",
  provSub: "Collapsed by default · keys never reach the client",
  provListHint:
    "Provider list mirrors the model configuration page ({n} total); expand a provider to switch adapters or add new ones.",
  fileLoadFail: "User file {f} failed to load: {msg}",
  noProvHint:
    "No configured provider found (adapters.json unreachable or none configured on the model page). Copy the guide prompt and let an agent wire up a data source.",
  customProvTitle: "Custom providers",
  customProvHint:
    "These providers are absent from the dsh model page (not selectable yet — register the adapter first; they count separately from the list above).",
  // 设置页：使用趋势
  trendTitle: "Usage trend",
  trendGranularity: "Granularity",
  trendGranDay: "Day",
  trendGranWeek: "Week",
  trendGranMonth: "Month",
  trendMetricLabel: "Metric",
  trendMetricTotal: "Total tokens",
  trendMetricInput: "Input",
  trendMetricOutput: "Output",
  trendMetricCacheRead: "Cache read",
  trendMetricCacheWrite: "Cache write",
  trendMetricCalls: "Calls",
  trendAdapterLabel: "Adapter",
  trendAdapterAll: "All adapters",
  trendByModel: "Split by model",
  trendCardTotal: "Window total",
  trendCardCalls: "Calls",
  trendCardPeak: "Peak bucket",
  trendCaliberNote: "Includes the ongoing bucket · tail bucket incomplete",
  trendCardTop: "Top adapter",
  trendCardTopDir: "Top directory", // directory-face top summary card label (mirrors trendCardTop)
  trendFetchFail: "Failed to load trend data",
  trendEmptyTitle: "No trend data yet",
  trendEmptyHint: "Trends for recent usage will appear here once sessions produce usage.",
  // Settings: usage trend
  trendRangeLabel: "Range",
  trendRangeDayUnit: "{n}d",
  trendRangeWeekUnit: "{n}w",
  trendRangeMonthUnit: "{n}mo",
  trendViewLabel: "View",
  trendViewBar: "Bars",
  trendViewArea: "Area",
  trendPartialOngoing: "In progress",
  trendPartialEdge: "Partial bucket",
  trendNoData: "No data",
  trendTipSum: "Total",
  trendPrevIncomplete: "Baseline incomplete",
  trendHiddenParts: "{k} part(s) hidden (cards use all parts)",
  trendLegendToggleHint: "Click a legend item to toggle its segment",
  trendPeakDay: "{day}",
  trendPeakWeek: "week of {day}",
  trendPeakMonth: "{y}-{m}",
  trendCardAvg: "Daily avg",
  trendViewVisible: "visible",
  // Settings: usage trend — directory dimension
  trendDirLabel: "Directory",
  trendDirAll: "All directories",
  trendDirUnidentified: "Unidentified",
  trendDirUnidentifiedNote: "Sessions without directory info",
  // 设置页：用量报告
  reportTitle: "Usage reports",
  reportPeriodDaily: "Daily",
  reportPeriodWeekly: "Weekly",
  reportPeriodMonthly: "Monthly",
  reportEnabled: "Enabled",
  reportTime: "Trigger time",
  reportWeekStartsOn: "Week starts on",
  reportWeekMonday: "Monday",
  reportWeekSunday: "Sunday",
  reportDayOfMonth: "Day of month",
  reportProvider: "Provider",
  reportProviderDefault: "Follow default",
  reportModel: "Model",
  reportModelHint: "Leave empty to follow default",
  reportModelDefault: "Follow default (first registered model)",
  reportModelKept: "{v} (current value, not in list)",
  reportModelFallback: "Model list unavailable or empty — falling back to manual input.",
  reportReasoningEffort: "Reasoning effort",
  reportReasoningUnset: "Not configured (follow DSH default)",
  reportReasoningDefault: "DSH default",
  reportReasoningStaleOption: "{v} (saved; not advertised by this model)",
  reportReasoningUnavailable: "This model does not advertise reasoning-effort capabilities.",
  reportReasoningCapabilityError:
    "Reasoning-effort capabilities are temporarily unavailable; the saved setting is preserved.",
  reportReasoningStale:
    "The saved reasoning effort is not advertised by this model; generation will fail closed.",
  // report directory scope multi-select
  reportDirectories: "Directory scope",
  reportDirectoriesAll: "All directories",
  reportDirectoriesSelectAll: "Select all",
  reportDirectoriesClear: "Clear",
  reportDirectoriesEmpty:
    "No directory data yet — directories appear here once sessions produce usage.",
  reportDirectoriesHintAll: "All directories are included (default scope).",
  reportDirectoriesHintScoped:
    "Reports include the directory breakdown of selected directories only (other dimensions stay full).",
  reportPromptReset: "Reset to default",
  reportPromptHint: "Edit by section, save as a whole; hard constraints folded by default.",
  reportNoData: "No usage in this period — nothing generated",
  reportHeroTotal: "tokens",
  reportHeroCalls: "calls",
  reportHeroActive: "active days",
  reportHeroStreak: "longest streak",
  reportHeroPeak: "peak day",
  reportRatioFlat: "flat",
  reportRatioPct: "{n}%",
  reportPrompt: "Prompt template",
  reportPush: "Push after generation",
  reportSave: "Save configuration",
  reportSaved: "Saved",
  reportSaveFail: "Save failed: {msg}",
  reportGenerate: "Generate now",
  reportGenerating: "Generating…",
  reportGenerateFail: "Generation failed: {msg}",
  reportStillGenerating: "Still generating in background — refresh the list later",
  reportReused: 'Report already exists for this window — reused (check "Regenerate" to overwrite)',
  reportForceRegen: "Regenerate (overwrite)",
  reportHistory: "Report history",
  reportEmpty: "No reports yet — configure a schedule or generate one now",
  reportFilterEmpty: "No reports under this filter — switch filter or view all",
  reportFetchFail: "Failed to load report data",
  reportOk: "ok",
  reportFailed: "failed",
  reportCollapse: "Collapse",
  reportDetailTokens: "Generation cost: {n} tokens",
  reportPeriodSelect: "Select period",
  // #940 P0: collapsible sections + structured prompts (batch 1: four report zones + history page)
  reportSectionSchedule: "Schedule",
  reportSectionRouting: "Routing & scope",
  reportSectionPrompts: "Report prompts",
  reportSectionGenerate: "Manual generate",
  reportSectionHistory: "History",
  reportExpandAll: "Expand all",
  reportCollapseAll: "Collapse all",
  reportUnsaved: "Unsaved",
  reportSectionOff: "off",
  reportSummaryNDirs: "{n} dirs",
  reportSummaryNReports: "{n} reports",
  reportGoalLabel: "Goal",
  reportStatsLabel: "Stats data",
  reportStructureLabel: "Structure",
  reportConstraintsLabel: "Hard constraints",
  reportConstraintCount: "{n} constraints",
  reportBudgetLabel: "Budget",
  reportPromptWordUnit: " chars",
  reportStatsVarNote: "{stats} is the stats JSON injection point (read-only, do not rename)",
  reportPromptRawHint: "Non-standard template — editing as a whole, no text lost",
  historySub: "Expand for detail · loaded on demand",
  reportFilterStatus: "Status",
  reportFilterAll: "All",
  reportFilterNoData: "Empty window",
  reportLoadMore: "Load more",
  reportForceShort: "Force",
  reportNoForceShort: "No force",
  reportGenIdempotentHint:
    "Reuses the existing successful report for the window by default; check Force to overwrite.",
  reportRangeVarNote: "{range} is the window-range injection point (read-only, do not rename)",
  reportWeakModelNote:
    "Monthly reports are long; weaker models may degrade or fail — retry, or switch models and regenerate.",
  reportRetryHint: "Generation failed — retry; check “Regenerate” to overwrite an existing report.",
  reportRetryRunning: "Report attempt is running; costs below are cumulative for this cycle.",
  reportRetryBusy:
    "A report attempt is already running; costs below are cumulative for this cycle.",
  reportRetryDeferred: "The current attempt did not finish; another automatic retry is scheduled.",
  reportRetryTerminal: "Automatic retries stopped: {code} ({kind}).",
  reportRetryNextAt: "Next automatic retry: {at}",
  reportRetryAttempt: "Current report attempt {attempt}; automatic retry limit {maxAttempts}.",
  reportRetryInputTokens: "Cumulative input tokens: {value}",
  reportRetryOutputTokens: "Cumulative output tokens: {value}",
  reportRetryReasoningTokens: "Cumulative reasoning tokens: {value}",
  reportRetryTotalTokens: "Cumulative total tokens: {value}",
  reportRetryCacheTokens: "Cumulative cache tokens: {read} read, {write} written",
  reportRetryDuration: "Cumulative duration: {value} ms",
  reportRetryUnknown: "not reported",
  reportHeroPeakHour: "peak hour",
  reportPeakHourValue: "{n}:00",
  reportGeneratedNotVisible: "Report generated but not in the list yet — reload to check.",
  reportReloadList: "Reload",
};
