/**
 * dsh-provider-usage — server/shared 趋势数据原语叶子（#768 A波3/5/6，同一下沉符号族一次搬完）。
 *
 * canonical 落点（由 server/collect/types.ts + server/aggregate/aggregate-query.ts
 * metricValue 纯下沉，表达式逐字一致，零行为变更）：
 * TREND_DIR_MAX / TREND_ROW_VERSION / TREND_UNIDENTIFIED / sumToken / safeToken /
 * safeId / sanitizeDirName / hourOfDay / isValidShardRow / 行类型族 / TrendCell /
 * TrendMetric / metricValue。
 * 共享层准入：无单一所有者（config 目录范围 + ui-routes 趋势过滤 + aggregate 压实/
 * 查询 + execute 快照/目录候选多方消费）、零值依赖（零 import）、≥2 正当消费者。
 * 目录外经 server/shared/interface.ts 消费，旧址保留 re-export 门面
