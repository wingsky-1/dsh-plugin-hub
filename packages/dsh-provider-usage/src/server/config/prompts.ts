/**
 * dsh-provider-usage — server/config 域：提示词表（#768 D1）。
 *
 * 本文件只回答「模板文本是什么」：现行三周期默认 + LEGACY 旧词锁表。
 * 锁表 = 迁移判定基准（用户是否自定义过旧模板的唯一依据），文本勿改动——
 * 改一个字即改变「旧默认→新默认自动升级」的判定面（归一化 normalize.ts 与
 * 迁移 upgrade/config-morph.ts 双消费，单物理定义）。
 * 复用边界：upgrade 经本域门面只复用词表纯数据（比较用），不调用归一化行为；
 * 归一化单答案仍归 normalize.ts（运行时读面）。
 */
import type { ReportPrompts } from "./shape.ts";
