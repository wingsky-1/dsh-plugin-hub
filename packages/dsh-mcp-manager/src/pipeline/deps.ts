 ...
 * workspace 域的全名解析与裸名回落（真实消费点：`bareServerName` 三处、`parseFullServerName`
 * 一处）。宿主能力实测 0 命中（无 ctx/Context/logger/settings），故只有一条 Port——比 §3.4 表的
 * 预期「workspace + 宿主 logger」窄，按实测收窄。
 *
 * 口径取「实际使用」而非「import 面」（附录 E.6 第 3 条的建议）：端口成员 = 域内真实的消费点。
 * `fullServerName` 本域从不引用（authorize 只 import `parseFullServerName` / `bareServerName`），
 * 端口不为零消费者的符号开口；跨端契约常量 MIDDLEWARE_GLOBAL_ROOT 已按 §3.6 规则 6 迁
 * `src/shared/constants.ts`（W3b），故不在端口面内。
 *
