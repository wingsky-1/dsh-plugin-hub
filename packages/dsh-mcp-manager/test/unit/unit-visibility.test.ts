 ...
 *
 * 覆盖七条判据：名单只取本方单元的 mcp__*（含反例）、名单未变不重挂（防自激循环的两道闸）、
 * 后注册收敛且旧限制先 dispose、空名单不调用 restrict、disposed/域 disposer 的清理、
 * 事件面缺失的降级、装载时的初始全量 reconcile。
 *
...
function fakeAgent(id, log, onChange) {
  let seq = 0;
  return {
    id,
    tools: {
...
function makeEvents(liveAgents = () => []) {
  const handlers = { created: [], disposed: [], changed: [] };
  const unhooked = [];
  return {
    handlers,
    unhooked,
...
/** 单元表夹具：本域只读 `unit.connections[*].id`（注册名 `mcp__<id>__` 的前缀来源）。 */
function unitsWith(...ids) {
  return new Map([["/proj", { connections: new Map(ids.map((id) => [`srv-${id}`, { id }])) }]]);
}
