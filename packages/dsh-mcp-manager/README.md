...
DSH 官方 importer 会按同名 section 直接导入 `dsh-mcp-manager`；本插件不再运行第二套 namespace migration。`settings.yaml.imported` 仅为已消费文档的审计副本，不应整体复制。
...
- **读不到当前会话时保持既有绑定、不做清空**（#1028）：官方会话快照就绪前、或该页签
  ...
  `no active project session`。告警口径：**首帧未知是官方快照尚未就绪的竞态
  （每次冷启动必现），不告警**；只有「已解析过又读不到」或「连续多帧仍读不到」才打
  一次告警，以免正常态与异常态显示同一条黄警而丢掉判别价值。
- 依赖 Node ≥ 20
