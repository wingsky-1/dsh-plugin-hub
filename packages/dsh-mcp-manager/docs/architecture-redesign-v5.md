...
6. **当前序列（路径 B：先 wire、后搬迁）**：**W1 连接域同域收口 ✓（`770859b`）** ~~+ W2 删 `directImpl`~~（**并入 W4**，G21）→ W3 共享常量迁 `src/server/shared/constants.ts`（**起点实测 3/2/32**，非计划记的 1/1/31；`isSharedLayerModule` 已含 `server/shared/**`，指向它的值边被豁免）→ **W4 catalog 端口 + 删 `connection/interface.ts:12` 复导**（两类环归零 → 基线才可写） → W4 catalog 端口（**两类环归零**）→ W5 pipeline → W6 inject（新建 `inject/deps.ts`）→ W7 `connection/orchestrator/deps.ts` → W8 `connection/runtime/deps.ts` → W9 `api/deps.ts` → W10 connection 门面 26 符号值面收口 → **W11 = B2b 原子变更**。每刀的实测数字见 D.2·39；**W7/W8 必须先登记 mutate 路径**（D.3·23）；**所有 `installXxx(deps)` 的字面调用点必须在 `src/index.ts`**（附录 G·G20）。
