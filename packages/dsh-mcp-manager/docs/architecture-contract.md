...
>
> **状态**：本文档是 **#664（上一轮 P0–P8 分阶段重构）** 的契约与门禁策略，**已被 `architecture-redesign-v5.md` 取代**为裁决面。本轮不改写正文，只按 §8.5 的三分类**逐节打标**；**正文的结构性合并与归档由 B2/B3 执行**（B2 删除 `bootstrap/` 与 `McpManager` 时同步；B3 收口）。
>
