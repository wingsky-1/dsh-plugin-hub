...
JEV decision gateway: frozen asking guides (English) + dual-track keys + local secret-shape precheck + official SystemOne calls (transport via the official `@typesafe-ai/sdk`, inlined at build time; base URL and model are both pinned, zero runtime dependencies).
