...
export const zh = {
  ...
  caCertPlaceholder: "留空 = 不下发 CA（自签模式下发叶子）",
  caDownload: "移动设备安装证书",
  caDownloadLink: "下载证书（.cer）",
  caDownloadHint:
    "iPhone 用 Safari 点下载：安装描述文件后去“设置→通用→关于本机→证书信任设置”打开信任；Android 在“设置→安全→安装 CA 证书”（自包 WebView 需放行用户 CA）；Windows 双击 .cer 装进“受信任的根证书颁发机构（本地计算机）”。装完重开即零警告；自定义叶子且无 CA 时链接 404（属正常，按提示配置 CA）。",
