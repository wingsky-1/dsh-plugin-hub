...
describe("接缝面锁定", () => {
  it("SETTINGS_NS 为稳定的 dsh-* settings namespace", () => {
    expect(SETTINGS_NS).toBe("dsh-lan-proxy");
  });
  ...
  it("服务级写面按条目 id 寻址（错 ns 拒绝）", async () => {
    ...
    expect(calls).toEqual([{ ns: "dsh-lan-proxy", patch: { port: 1 } }]);
    await expect(service.update("ui-dsh-lan-proxy", { port: 1 })).rejects.toThrow();
  });
});
