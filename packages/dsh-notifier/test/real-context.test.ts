...
// ── 真实 Context 三断言（未注入访问抛错 / ctx.get 安全 / effect disposer）──
{
...
// ── 真实 Context 派发 agent/status 可达 + fail-closed ──
{
  const root = new Context();
  let pluginCtx = null;
  await root.plugin((ctx) => {
...
// ── 真实 Context 派发 session/event 可达 + fail-closed ──
{
  const root = new Context();
  let pluginCtx = null;
  await root.plugin((ctx) => {
...
// ── 源码静态契约断言（test 内扫描，杜绝根因 A 与 {global:true} 遗漏回归）──
{
  const src = readFileSync(join(import.meta.dirname, "../src/index.ts"), "utf8");
  // isSubagentOf 不得直读未注入的 ctx.agents；必须走 ctx.get 安全读取
  ...
  // 全部 ctx.on 注册处均带 { global: true } 第三参数（当前主干 7 处；
  // approval/request 一处升级为 { global: true, prepend: true } 链序契约）
  ...
  // approval/request 注册处必须带 prepend: true（链头契约）
  assert.ok(
...
// 链头先执行、outcome 透传）。
{
  // 对照基线——push 序下先注册的短路 answerer 独占执行（回归防线：
  // 若 cordis 语义变化使 prepend 失效，本断言与修复行为的差异会同时暴露）
  {
    const root = new Context();
    const order = [];
    await root.plugin((ctx) => {
