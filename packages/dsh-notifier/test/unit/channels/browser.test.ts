...
describe("sendBrowser", () => {
  ...
  it("铃声决议：缺省与 false 都静音，跟随系统按弹不弹分叉，显式音色走自播", () => {
    const { frames, targetOf } = collector();
    const first = sendBrowser(targetOf({ sound: undefined }), messageOf());
    sendBrowser(targetOf({ sound: false }), messageOf());
    sendBrowser(targetOf({ sound: true, pop: true }), messageOf());
    sendBrowser(targetOf({ sound: true, pop: false }), messageOf());
    sendBrowser(targetOf({ sound: "ding", pop: true }), messageOf());
    ...
    expect(frames.map((frame) => frame.pop)).toEqual([true, true, true, false, true]);
  ...
  it("按页面侧上限截断标题与正文，severity 缺席就不写这个键", () => {
    const { frames, targetOf } = collector();
    sendBrowser(targetOf(), messageOf({ title: "题".repeat(100), body: "文".repeat(3000) }));
    sendBrowser(targetOf(), messageOf({ severity: "warning" }));
