/**
 * dsh-notifier api 域 —— 动态种类端点：清单与确认。
 *
 * 两个动作都走 sdk 域的管理面（能力面的 `listKinds` / `confirmKind`），本块不自己读设置、
 * 不自己写名单：确认态的写入路径只能有一条，写两份的代价不是多写一次，而是两处对「已确认」
 * 的理解可能出现分歧——而分歧的表现是「设置页上点了允许，通知还是不发」。
 *
 * 存在性在本块判（未登记 → 404）而不是在 sdk 域：那是能给用户答复的层次，而域内的守卫
 * 抛错不是给用户看的。
 *
...
export class KindEndpoints {
  ...
  /**
   * POST /kinds：确认 / 撤销一个动态种类。
   *
   * 四态逐态映射而不是压成一两个状态码：未登记是 404（客户端在提交一个已经消失的种类）、
   * 参数非法是 400、版本冲突是 409、服务不可用是 503。压扁之后用户看到的就只剩「操作
   * 失败」，而这四种原因要做的事完全不同。
   *
 ...
 * 写面结果 → 响应。
 *
 * 与设置端点同款映射：同一个写面出来的失败，在两个端点上给出不同的答复，只会让客户端
 * 按路径分叉处理同一件事。
 */
