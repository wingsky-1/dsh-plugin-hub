 ...
 *
 * 把散在 DSH home 根目录的数据文件收进包私有目录。**没有旧文件的安装同样在这一步落定**：
 * 直接建出初始形态，让目标目录一次到位，而不是留一个空目录等各域自己想起来补——
 * 「存储在哪、长什么样」这个问题，应该在升级这一步就有答案。
 *
 * **配置文件不在这里**：它的旧形态有两代（0.2.3 的官方 settings 命名空间、更早的自建
 * JSON 文件），而读取前者要等宿主服务就绪——那是装配期拿不到的时机。它由 config 域在
 * 服务就绪后自己取一次，代价是这里不再预建配置文件（第一次写入时建，没有区别）。
 *
 * 初始形态一律是**空结构**而不是默认值快照：设置文件写全量默认值会把「用户覆盖
 * 过哪些键」这个语义冲掉（文件里的每一个键都会被读成用户显式提交的），而这份差别
 * 正是设置页判断哪些字段该标记为「已改」的依据。
 *
 * 逐文件独立：一个搬不动不影响其余——几个文件之间没有依赖。**搬不动都抛出**：迁移没
 * 做完而启动照常，等于让各域按错误的形态去读数据，而那正是最难查的一类故障。
 *
