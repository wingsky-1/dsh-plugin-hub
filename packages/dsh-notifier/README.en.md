...
Error mapping (POST /kinds): kind-confirmation CAS retries (≤2) exhausted → 409 (`code:"SETTINGS_CONFLICT"`, rare: sustained concurrent writes during confirmation); write failure → 500 (fixed `error` text, root cause only in server logs). The 200 response carries the fresh `revision` for the client to sync its optimistic-concurrency version.
