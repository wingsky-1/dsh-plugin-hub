...
<a id="deployment--how-to-access-important"></a><a id="user-content-deployment--how-to-access-important"></a>
...
- **System-channel availability lives in `/diagnostics`** (probed on the host, with
  remediation advice); **browser-channel availability is computed in the settings card**
  (client-side, so it differs per device).
...
| `/api/dsh-notifier/status` | GET | Latest channel outcomes and failure counts. |
| `/api/dsh-notifier/kinds` | GET / POST | Read and confirm dynamic event kinds. |
| `/api/dsh-notifier/health` | GET | Health and capability summary. |
| `/api/dsh-notifier/diagnostics` | GET | Detailed host capability probe. |
...
SSE notification frames (browser EventSource subscription; `?since=<seq>` replays missed frames after reconnect)

#### `/test`

Test notification (funnels through the service pipeline, bypasses Do-Not-Disturb; optional body `{channelId}` to test a single channel)

#### `/history`

GET recent notification records (up to 200, filtered by `historyMaxAgeDays`; entries suppressed by DND are flagged `suppressed`; every record carries per-channel delivery detail `channels[]` whose `reason` is a structured reason); **DELETE clears**

#### `/status`

Channel delivery status (per-channel latest delivery terminal state + consecutive failure count; the failure reason is a structured object `{code, params?, detail?}` whose `detail` is truncated as-is to 300 chars, with no credential replacement)

#### `/kinds`

GET the dynamic kind list (including confirmation state); POST `{kind, confirmed}` writes a confirmation (persisted to `allowKinds`); the 200 response carries `revision` (for the client to sync its optimistic-concurrency version)

#### `/health`

Health check (`{ok, plugin, platform, sseEvicts, capabilities}`; `capabilities.host` is a **summary** — the verdict plus the state of the two dimensions, constant size)

#### `/diagnostics`

Full capability self-check (`capabilities.host` with per-dimension `checked`, the player list, tone-file availability and the `remediation` advice). It **shares the one probe** with `/health` (one probe per process, cache reads afterwards); the **first** probe may wait up to the 8s total budget (`CAPABILITY_BUDGET_MS`) — a known cost
...
- **Do-not-disturb windows (multi-window as of 0.2.6)**: up to 5 windows (`quietHours.windows`); hitting any one suppresses (union semantics, overlaps allowed), an empty array means no hit; a single window supports crossing midnight (e.g. 22:00 → 08:00); an **urgent exception** can be set (`quietHours.allowKinds`: events still reminded during DND). The default candidates are the high-frequency blocking kinds (approval / question / error); the settings page lets you check **all 6 built-in events** (including task-done / subagent-done / turn-end) with one-click "Follow enabled events" or "Reset default". Exemption is orthogonal to the event toggles — a disabled event never produces notifications anyway, and the exemption entry stays intact; disabled events are shown dimmed (reduced opacity) on the settings page and can still be exempted. The settings page adds/removes rows and previews the current hit from this device clock (preview for reference only; the server verdict and history rule). **Upgrade note (0.2.6)**: legacy `start`/`end` are moved into `windows[0]` at assembly time and the old keys deleted; submitting the old shape after the upgrade always returns 400 with a refresh hint. **Older upgrade note**: now that the allow-list is open, kinds that older configurations used to filter out (e.g. hand-edited `done`/`turn-end`) will be reminded again during DND — a behavior change; adjust the exemptions on the settings page if you do not want that
...
- **Host capability self-check (as of 0.2.4)**: the **body of the system card** shows the host-channel verdict (whether popup / sound work, plus any dimension that cannot be determined), and lists remediation advice when something is unavailable (which package to install, or switch to the browser channel); the detail (which dimensions were probed, which players were found) is collapsed. It deliberately stays out of the **card header**, which is hidden on narrow screens — exactly where "why did nothing ring" matters most. The **body of the browser card** shows the browser-channel verdict, which is computed **on this device**, so it differs per device. Both come from `GET /diagnostics`
...
When subscribing to host events (`approval/request`, `user-questions/request`, `session/event`,
...
Tests are organized under `test/{unit,integration,e2e,client,client-dom,client-unit}/`: host unit and integration tests, e2e smoke tests, client artifact checks, DOM tests and client unit tests. Mutation testing reuses the relevant assertions through the lib→src hook; these layers do not imply real-machine coverage on every platform.
...
| Linux system notification: real `notify-send` invoked and its exit code asserted | e2e, platform marker | **Conditional coverage**; prerequisites below. |
...
**Real Linux notification prerequisites**: **Conditional coverage**: it actually runs only when the test machine has a real `notify-send` on the system PATH **and** a D-Bus session is available (`DBUS_SESSION_BUS_ADDRESS` set, or `$XDG_RUNTIME_DIR/bus` present); otherwise it skips with a reason. Whether it is covered therefore depends on the runtime environment and is not inferred here
