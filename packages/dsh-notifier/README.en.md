...
- **Host capability self-check (as of 0.2.4)**: the **card body** of that same card shows a line with the host-channel verdict (whether popup / sound work, plus any dimension that cannot be determined), and lists remediation advice when something is unavailable (which package to install, or switch to the browser channel); the detail (which dimensions were probed, which players were found) is collapsed. The browser-channel half is computed **on this device**, so its verdict differs per device. The data comes from `GET /diagnostics`
