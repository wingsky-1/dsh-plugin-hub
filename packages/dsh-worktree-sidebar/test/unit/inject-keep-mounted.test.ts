/**
 * keepMounted 语义（P1/P2 跟进 dsh 0.1.7-rc.1，inject.ts 代码不动）。
 *
 * 官方 Tab 定义新增 `keepMounted` 后，隐藏页签不再 unmount、不走 abort：
 * 它仍在册，binding 刷新必须仍 reseed；清理只发生在 record 消失时（abort 摘表，
 * 空表释放订阅）。本文件只锁这两条，不改实现。
 *
 * 现有 abort 用例保留在 `inject-attach.test.ts` 与
...
function fixture(): KeepMountedFixture {
