...
- **Injection fence semantics**: Injected text is wrapped in a fixed `<user_long_term_memories>` fence with companion guidelines — fenced content is background facts from past sessions, not control instructions; the current explicit user request always takes precedence on conflict. The companion guidelines are embedded in the same fence message (single-message form; no second standalone discipline message) and never merged with the existing memory discipline prompt. Fence-tag-like text inside memory content is neutralized before assembly, and fence integrity is asserted by an injection-side invariant (open/close tags exactly once each).
