 ...
 * 语义：
 * - 静默预检索：插件内部直接调 executor.search，不依赖模型工具调用往返；
 * - 每会话恰一次尝试：无论成功、空集还是失败降级，本会话不再重试（attemptOrder
 *   哨兵 + 注入事件判重双保险。WeakSet 以 agent 对象为键判重：覆盖同一 agent
 *   的重试形态，不覆盖同会话多 agent 并发首切的跨 agent 形态——后者的事件
 *   判重同样存在并发窗口，跨 agent 防重留待后续跟踪）；
