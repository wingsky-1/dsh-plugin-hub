/**
 * dsh-opencode-usage — 路径解析工具（M2 用户适配器注入）。
 *
 * 解析规则（同计划 3.2）：
...
/** 插件 home 目录（host 文件逻辑归属区 ~/.dsh/plugins/opencode-usage）。 */
export function pluginHome(): string {
  return join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "plugins", "opencode-usage");
}
